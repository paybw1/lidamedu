...
> **⚠️ 대체됨 (SUPERSEDED)** — 이 기능은 feat-8-023(주석 3종 통합 · 작성자
> 역할 기반 가시성)으로 대체되었다. 하이라이트형(앵커) 코멘트는 제거되었고,
> 강사 하이라이트의 학생 노출은 가시성 규칙이 직접 담당한다. 아래 내용은
> 이력 보존용이다. 상세: `docs/features/feat-8-023-annotation-visibility.md`
