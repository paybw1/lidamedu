...
- **모순 방어 가드**: 한 그룹에 O/X 가 섞여 있으면(`distinct_truths≥2`) **합치지 않고
  전부 노출**한다 — 정답 오류를 조용히 숨기지 않고 staff 가 보게. (현재 1건은 Phase 1
  에서 데이터 교정 → 런타임 모순 0.)
...
- **Phase 1** (데이터 소량, 승인 후): 모순 1건(제226조) 두 ref 전체 본문·해설 확인 →
  맞는 `ox_truth` 결정(법적 판단) → dry-run(before/after) → 승인 → 1 row 교정.
