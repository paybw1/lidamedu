...
- **화면 리허설(로컬 서버 + 운영 DB, 읽기 전용, `rehearse-q2-ui.mjs`, 6/6)**: 기본 목록 31행 전부 셀렉트 렌더(보관 24·환불완료 5·없음 2 = dry-run 과 일치) · 「보관」 선택 → 사유 다이얼로그(전→후·안내문·사유 필수) 열림(제출 안 함) · 보관함 필터 0건 렌더 · 페이지 오류 0. 임시 관리자는 무력화 계정 재사용 후 재무력화.
- 게이트: typecheck·vitest(655)·build 통과 → 로컬 커밋 5c1bc890 → 원장 「진행 OK」(2026-09-17) → 푸시(5c1bc890·fa13a589) → Vercel Ready → **운영 화면 리허설 6/6 동일**(31행 셀렉트·분포 일치·다이얼로그·보관함·오류 0, 읽기 전용). 실제 전이 실행 리허설은 아래.
- **전이 리허설(운영, 원장 지시 2026-09-17, `rehearse-q2-transitions.mjs`, 14/14)**: 임시 상품(온라인 상시·1,000원·`q2_rehearsal_1789624707129`) → 무통장 신청(`afa63c31`) → 취소(기한 종료·이력 1) → 입금대기(기한 72h 재설정) → 취소 → 보관(archived_at) → 보관 중 입금대기 거절(400) → 보관 해제 → 입금대기 → 사유 1자 거절(400) → 결제완료(paid·paid_at·deposited_at·수강권 active) → paid 취소·보관 거절(400) → 전액 아닌 환불완료 거절(400) → 환불완료 1,000원 계좌환불(orders refunded·refunds `fc05dbba` full_done·closed_at·항목 refunded_at·수강권 회수) → 이력 8건 순서 일치(cancelled→pending_deposit→cancelled→archived→unarchived→pending_deposit→paid→refunded, 처리자 = 임시 관리자) → 감사 로그 8건. **남은 행**: 임시 상품(판매중지·비활성, 신청내역 때문에 삭제 불가)·주문 1·무통장 신청 1·환불건 1·수강권 1(회수)·이력·감사 로그. 매출 카드에는 당일 총매출 +1,000·환불 +1,000 으로 잡힌다(순액 0). 운영 리허설(무통장 테스트 주문 생성 → 취소·입금대기·보관·결제완료 각 1회)은 주문·수강권 행을 남기므로 원장과 시점 협의.
