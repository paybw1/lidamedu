...
- 게이트: typecheck·vitest(655)·build 통과 → 로컬 커밋 5c1bc890 → 원장 「진행 OK」(2026-09-17) → 푸시(5c1bc890·fa13a589) → Vercel Ready → **운영 화면 리허설 6/6 동일**(31행 셀렉트·분포 일치·다이얼로그·보관함·오류 0, 읽기 전용). 실제 전이(취소·입금대기·보관·결제완료·환불완료) 실행 리허설은 아직 0회 — 첫 실사용은 관찰 하에. 운영 리허설(무통장 테스트 주문 생성 → 취소·입금대기·보관·결제완료 각 1회)은 주문·수강권 행을 남기므로 원장과 시점 협의.
