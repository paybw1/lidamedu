...
> 상태: ✅ P1~P5 완료 — V2 정식 전환(기본값). iPad 이월 검증 통과. 데스크톱(Windows IME)은 미검증(롤백 `?blankv1=1` 상시 가능).
...
- **P4**: 판례 빈칸 이식.
- **P5**: ✅ 정식 전환(2026-07-22) — V2 가 **기본값**(조문 뷰어·장 뷰어 loader `blankV2: !?blankv1`),
  "실험 렌더(v2)" 배지 제거. **구 모델은 삭제하지 않고 `?blankv1=1` 롤백 경로로 보존**(데스크톱
  Windows IME 미검증 대비). 기존 blank-fill E2E 는 `?blankv1=1` 로 구 모델 회귀 검증(V2 전용
  E2E 는 후속). ★남은 후속: 데스크톱 실사용 확인 후 문제없으면 구 모델·롤백 플래그 제거.
