...
| 경로 | 내용 |
|---|---|
| 상단 계정 아이콘 → 「정산현황」 | 팝업(데스크톱 Dialog / 모바일 Sheet). 강사·원장에게만 보인다. 열 때 한 번만 로드 |
| `/lecture/settlements` | 같은 패널의 전체 화면. 확정·지급 알림의 링크 목적지 |
| `/api/lecture/settlement` | 팝업이 월을 바꿀 때 부르는 JSON + `?export=csv` |
...
5. **도서 정산은 쿠폰 할인을 안분하지 않는다**(`book-settlements-admin.server.ts` 는 `unit_price_krw × quantity` 그대로). 강사 정산과 기준이 다르므로 도서 쪽도 맞출지 원장 판단 필요.
