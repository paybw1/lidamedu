...
| 단계 | 내용 | 하드스톱 |
|---|---|---|
| **A ✅** | 마이그레이션 + 백필 + 트리거 — 링크 2,003(전 과목 대표 1,998 + 상표 다중 5). 상표 364 링크 전부 주제별 본문 보유. 트리거 양방향 실측 확인. | 완료 |
| **B ✅** | 읽기 경로 전환 — `getCasePlacementMaps`(트리 카운트)·`getCaseIdsByPlacement`(목록 필터)가 링크 union 을 본다. 링크 없는 판례는 `primary_node_id` 폴백. 실측: 주제18 7→8 · 19 13→14 · 27 10→11 · 32 5→6 · 40 10→11, 나머지 42개 주제·타 과목 무변화. 주제19 목록에 96후1866 노출 확인. | 완료 |
| **C ✅** | 뷰어가 `?node=` 로 그 주제의 서술을 연다(`getCaseById(client, caseId, nodeId)` → 링크 book_sections 우선, 없으면 `cases.book_sections`). 목록 행 링크가 활성 주제를 실어 보내고, 주제 컬럼도 그 주제를 보여 준다. 본문 위 「교재 수록」 칩으로 두 자리를 오간다(배치 2개 이상일 때만). 회귀 테스트 3건. | 완료 |
| **D ✅** | 운영자 배치 관리 — `/admin/cases/edit/:id` 「교재 수록 자리」 카드(추가·대표 지정·삭제·자리별 본문 저장). intent 4종 `add/remove/set_primary/save_body_case_placement`. ★대표 배치는 삭제 차단(다른 자리를 대표로 지정한 뒤 삭제). ★카드는 메인 폼 **밖**에 둔다 — BookSectionsEditor 의 hidden `bookSections` 가 본 저장을 덮는다. 메인 저장은 대표 링크 본문을 자동 동기화. 회귀 테스트 2건. | 완료 |
