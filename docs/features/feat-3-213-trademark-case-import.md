...
- 상표 판례 **337건 = 교재 고유 사건번호 전량** (신규 insert 326 + 특허 판례집 겹침 11건은
  기존 row 의 `subject_laws` 에 trademark append — 배치는 특허 노드 유지, 상표 트리에선 미배치)
- 평석 54건, 도형 이미지 694장 업로드(webp) — 변환 불가 잔여 4장(WMF 2·OLE 2)
...
- **[중요] 특허 판례 이미지가 구 프로젝트(nctokynz) 스토리지 URL** — 현재 200 응답이나 구 프로젝트
  중지 시 일괄 깨짐. `case-images` 재업로드 + `cases.images` URL 치환 마이그레이션 필요.
- 교재 내 중복 수록 4건(2017나1148·96후1866·2002후567·2000후3708)은 최초 주제에만 배치 —
  두 번째 주제에서도 보이게 하려면 다중 배치 모델(현재 미지원) 필요.
- 특허 겹침 11건(2002후567·2018다221676·2023다280358·2018도14446·2016후2317·98후300·2004후387·
  2012후2432·2008후4486·2007후4649·2015후1997)은 상표 트리에서 미배치(맨 뒤) — 상표 쪽 배치를 원하면
  `/admin/cases/edit` 에서 primary 노드 지정(단, 특허 배치와 택일).
- WMF 2·OLE 2·TMP 9 이미지 형식은 변환 skip(해당 판례는 이미지 없이 적재, 로그 확인).
