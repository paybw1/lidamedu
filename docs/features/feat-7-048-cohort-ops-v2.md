...
### Stage D · 오늘 학습 기록 개편 — ✅ 완료 (2026-08-21)

검증 `tmp/feat7048-verify/stageD.itest.ts` 5/5 + 기존 `tmp/phase3-verify/` 전량 재실행 21/21(append-only·RLS 회귀, 테스트 데이터 잔여 0) · typecheck ✅ · build ✅. 마이그레이션 없음 — Stage C 의 M3 컬럼을 쓴다.

작업 중 고친 표시 결함 하나 — **지난 버전 항목에 달린 기록이 목록에서 사라지고 있었다**(Phase 3 부터). 항목이 살아 있어 `plan_item_id` 가 유지되는데 오늘의 기대 항목에는 없어서, 기대 카드에도 계획 외 목록에도 안 잡혔다. 합계·타일에는 들어가 있으니 눈에 보이는 합과 어긋난다. 이제 **오늘의 기대 항목에 대응하지 않는 기록은 전부 계획 외 목록**으로 내려온다.

미래 날짜는 **열람만** 가능하다 — 기대 카드의 완료·부분 버튼이 비활성이고, 계획 외 학습 폼도 열리지 않으며, 서버가 한 번 더 거부한다.
