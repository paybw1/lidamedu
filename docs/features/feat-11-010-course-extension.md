...
## 7. 단계 — 전부 완료 (2026-09-01)

- **A** ✅ DDL(운영 적용) + typegen + `lib/extension-policy.ts`(테스트 15) +
      영상 수강권 화면 상단 기본값 폼 + 상품 › 수강 정책 강의별 override
- **B** ✅ 마이페이지 [수강연장] 버튼 + `/api/lms/extension-order`(서버 재검증) +
      토스 결제창 + `markOrderPaidAndFulfill` 의 `course_extension` 지급
- **C** ✅ 환불 원복(`revokeItemFulfillment` 분기) + `/admin/lms/extensions` 이력 화면
      + 수동 처리 대기 배지·처리 기록

### 구현하며 정한 것

- **적용 시점에 다시 계산한다.** 주문 생성 때 계산해 둔 일수·만료일을 쓰지 않는다 —
  결제창에 머무는 동안 만료일이 바뀌었을 수 있고, 그때는 지금 값이 맞다.
- **결제가 끝난 뒤 정책이 꺼져 있어도 연장은 적용한다.** 돈은 받았는데 연장이 안 되는
  상태가 훨씬 나쁘다.
- **환불 원복은 만료일 빼기가 아니라 "일수 빼기"** 다. 그 사이 다른 연장이 더 붙었을 수
  있어서, 현재 만료일에서 그 결제의 일수만 뺀다(단, 원래 만료일 아래로는 내려가지 않는다).
- `logEnrollmentAdminAction.actorId` 를 nullable 로 넓혔다 — 환불 원복은 사람이 아니라
  시스템이 일으킨다(컬럼도 원래 nullable 이었다).
