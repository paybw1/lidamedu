...
- ★**D15 포인트 결제가 붙으면 환산 분모를 다시 봐야 한다.** 지금 `round(실환불액 × 정가 ÷
  결제귀속액)` 이 성립하는 것은 `paid_amount_krw`(토스가 실제로 청구한 몫)와 정산의 `netKrw`
  (`allocateDiscount` 산출)가 같은 값이기 때문이다 — 포인트 결제가 없어(Q=0) 둘이 갈릴 일이
  없었다. 포인트가 붙으면 `paid_amount_krw` 는 포인트를 빼고, 정산 `netKrw` 는 `.discount` 가
  무엇을 담느냐에 따라 달라질 수 있다. **둘이 포인트를 같은 쪽으로 다루는지 확인하고, 다르면
  환산 분모를 정산 규칙에 맞춘다.** 회귀 테스트는 양쪽에 같은 값을 넣으므로 이것을 못 잡는다.
