...
#### ★배포 시점에 원장이 정할 것 (2026-09-15 실측)

| 항목 | 실측 | 결정할 것 |
|---|---|---|
| **3일 창 안 학생** | **2명** (둘 다 45,000원·`active`, 창 만료 9/17 10:26·12:31 UTC) | 배포 순간 이 2명은 어제 있던 해지 버튼 대신 「고객센터로 신청 ☏」을 본다. 두 경로 다 살아 있어(전화·`refundPaymentAdmin`) 막히지는 않으나, **이용자에게 불리한 변경**에 해당할 수 있어 개별 안내를 할지 공지로 갈음할지 |
| **공지 표면** | `announcements` (`platform_scope=study`, `audience_kind=all`, 5건 게시 중) · `popup_notices` (오픈 할인 1건 활성, ~9/30) | `refund-policy.mdx` 스스로 「개정 시 서비스 내 공지합니다」라고 약속한다. 배포일에 공지가 나가야 그 약속이 지켜진다 — 문안은 원장 몫 |
| **시행일 표기** | `refund-policy.mdx` 에 개정일·시행일 줄이 **없다** | 학생이 규정이 바뀐 사실 자체를 알 수 없다. 줄을 넣을지, 넣는다면 시행일을 언제로 할지 |
