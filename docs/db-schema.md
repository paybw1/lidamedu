...
create table public.problems (
  ...
  main_case_number    text          -- 2026-08: 주관식 관련판례 메인 지정. 복수 가능(", " 구분 사건번호 목록, 설문별 메인). 배지 그룹 맨 앞+★ 강조. 지정=뷰어 배지 팝업 토글(staff)
);
...
## offline_tests / offline_test_questions / offline_test_results / offline_test_answers  ✅ 적용됨 (2026-07-05 feat-7-042 · 2026-08-13 Phase 1)

오프라인 시험지 — 종합반 과제(assignment)에 붙는 빈칸·OX·객관식 조합 테스트.

- **offline_tests**: test_id PK, assignment_id FK(cascade), cohort_id FK, title, law_code(5법 check) XOR science_subject, **status check('draft'|'published'|'closed') default 'draft'** + published_at/closed_at(Phase 1 T2 배포 게이트 — draft=학생 비노출·편집 가능 / published=노출·문항 잠금·결과 입력 / closed=결과 열람만), duration_min, instructions_md, series_id/series_round_no(feat-7-044), created_by, soft-delete(deleted_at).
- **offline_test_questions**: test_id FK, ord, points numeric(5,1), question_type check('mcq'|'ox'|'blank') + **유형별 참조 XOR check** — mcq=problem_id / ox=ox_ref_type('choice'|'box')+ox_ref_id(폴리모픽, FK 없음)+ox_problem_id / blank=blank_set_id(article_blank_sets, 세트 전체=1문항). ord 는 일반 인덱스(UNIQUE 아님 — 코드 규칙 유지).
- **offline_test_results**: (test_id,user_id) unique, status('taken'|'absent'), score/max_score, session_id(신호 합류로 만든 학생 명의 quiz_session, set null), **srs_problem_applied_at/srs_ox_applied_at**(Phase 1 S1 — SRS 축별 1회 적용 마커, 정정·철회에도 재적용/되돌림 없음), taken_at date, entered_by. ※ wrong_ords int[] 는 Phase 1 T1 에서 offline_test_answers 로 대체(검증 후 제거 예정).
- **offline_test_answers**(Phase 1 T1): PK(result_id FK cascade, question_id FK cascade) + is_correct — 문항별 정오 스냅샷, 키=question_id(불변)라 문항 편집이 스냅샷을 오염시키지 않음. N2(선택답·부분점수)는 컬럼 추가로 확장.
...
- **feat-7-048 Stage B**: (상담자가 직접 쓴 계획이면 그 staff id — 학생 제출과 갈라 세는 근거,  제출률 분리 집계) · . ★staff RLS 를  → select(넓게) + insert/update/delete()로 **분리**했다 — 종전에는 staff 가 승인본까지 고칠 수 있었다(화면이 없어 드러나지 않았을 뿐). 승인은 security definer RPC 라 이 정책에 걸리지 않는다. 알림 kind  추가(★ STUDENT_KINDS 등록 필수).
- **feat-7-048 Stage E(2026-08-21)**: 신규 study_timer_sessions(사용자당 진행 중 1개 = 파셜 유니크 where ended_at is null · paused_ms 누적 + paused_at · 종료 시 study_logs 로 INSERT 하고 log_id 연결). ★세션 행은 UPDATE 가능하지만 **확정된 학습 기록은 여전히 append-only** 다 — 진행 중 상태를 원장에 두지 않으려고 테이블을 분리했다. 서버는 시각만 갖고 경과는 study-plans/lib/timer.ts 로 재계산한다(서버리스라 타이머를 못 돌린다). 자정을 넘긴 세션은 벽시계 비율로 날짜 분할, 12시간 초과는 자동 확정 대신 사람 확인. study_logs.source 에 timer 추가. 신규 student_study_prefs(record_mode timer|total, **기본 timer** — 2026-08-21 원장 결정으로 total 에서 변경). 둘 다 RLS 본인 R/W + staff SELECT.
- **feat-7-048 Stage C(2026-08-21)**: 과목 축 `study_plan_items.subject_kind/subject_code` + `study_logs.subject_kind/subject_code`(★INSERT 시점에만 채운다 — append-only 라 UPDATE 로 메우지 않고, 과거 로그는 조회 시 `plan_item → node → law_code` 파생. 통계는 `저장값 ?? 파생값`) + `study_logs.started_at/ended_at`(Stage E 타이머·총량 입력의 시작 시각. NULL = 시각 미지정). 값 집합 SSOT = `study-plans/subject-axis.ts`. 신규 `student_subject_colors(user_id, subject_kind, subject_code, color_key)` — **팔레트 키만 저장**(hex 금지, 다크 모드 정합). RLS 본인 R/W + staff SELECT.
- **feat-7-048 Stage B(2026-08-21)**: `study_plans.authored_by`(상담자가 직접 쓴 계획이면 그 staff id — 학생 제출과 갈라 세는 근거, `scripts/ops/phase3-gate-metrics.mjs` 제출률 분리 집계) · `study_plan_items.updated_by`. ★staff RLS 를 `FOR ALL` → **select(넓게) + insert/update/delete(`status <> 'approved'`)로 분리**했다 — 종전에는 staff 가 승인본까지 고칠 수 있었다(편집 화면이 없어 드러나지 않았을 뿐). 승인은 security definer RPC(`approve_study_plan`)라 이 정책에 걸리지 않는다. 상담자 편집 흐름 = in-flight 있으면 그 행 편집 / 승인본만 있으면 v+1 draft / 없으면 새 draft(`ensureEditablePlan`), 승인은 **draft→submitted(대신 제출) → 기존 RPC** 두 문장. 학생 알림 kind `study_plan_updated_by_staff` 추가(★`notifications/kinds.ts` STUDENT_KINDS 등록 필수).
...
- **books / book_stock_moves / v_book_stock**: 재고=append-only 원장 SUM 파생. plan_book_links(필수/선택). **shipments**(order_item 1:1, 본인 RLS self-read — 관리자 갱신 즉시 마이페이지 반영).
  - **books.subject_code** (2026-09-11 과목별 진열, `scripts/sql/20260911_books_subject_code.sql`): text null, CHECK ∈ patent/trademark/design/civil/civil-procedure/science — 강의개설·강의그룹과 같은 코드 집합(화면 SSOT `app/features/lms/lib/subject-options.ts`). null=미분류(도서몰 「기타」 묶음 맨 뒤). 도서몰·도서 관리 진열 = 과목 순서 → `sort_order`(동률 최신순)이므로 **sort_order 는 같은 과목 안에서만 의미**(화살표 이동도 묶음 내). 초기값은 제목(특허법/상표법)으로 백필. ★1차/2차 구분(exam_round)은 원장 판단 보류.
  - **books.cover_file_path / cover_path** (★2026-09-17 feat-11-015 §3): `cover_file_path` 값 = 업로드 후 **완성 공개 URL**(`https://…/book-covers/covers/<uuid>.jpg` — 버킷 경로가 아님, `bookstore/lib/book-fields.server.ts` 가 `getPublicUrl(path).data.publicUrl` 을 저장). `cover_path` = 외부 URL 직접 입력. 소비처는 전부 `pickCover(cover_path, cover_file_path)`(`bookstore/queries.server.ts`, export) 로 읽고 **`getPublicUrl` 을 다시 붙이지 않는다** — 붙이면 이중 프리픽스(`…/book-covers/https://…`)로 400(강의 상세 「함께 쓰는 교재」 실사고).
...
- **book_settlements / book_settlement_items** (feat-8-029 P6): 도서정산 계산·지급(강사 instructor_settlements 와 동형). payee(저자/출판사)×월 unique, draft→confirmed→paid. 월별 도서판매 order_items 에 배분규칙 적용→payee별 정산, draft 재생성/confirmed·paid 불변, 이중계상 방지((payee,order_item,kind) 전기간 대조), 확정분 환불은 익월 refund_adjustment 음수차감. 배분=percent(기준액×%)/fixed(권당×수량, 부분=비례). RLS enable+정책無(adminClient 전용). ★기준액=**정가**(쿠폰 할인 미반영) — 강사정산(할인 후)과 다르지만 **원장 결정 2026-09-12: 다른 것이 맞다, 통일 금지**.
- **settlement_settings / instructor_settlement_profiles** (feat-8-031): 강사 정산 파라미터. `settlement_settings` 는 단일 행(id=1) `pg_fee_rate_bp`(결제 수수료율 bp, **0=미설정**). `instructor_settlement_profiles` 는 강사별 `tax_type`(withholding/invoice/none)·`tax_rate_bp`, **행이 없으면 개인 원천징수 330bp(3.3%)**. 둘 다 RLS enable+정책無(adminClient 전용). 정산 생성 시 정산서에 스냅샷 복사 → 확정분 지급 근거 불변.
- **instructor_settlements / instructor_settlement_items 확장** (feat-8-031): 원천이 둘이 됐다 — item 은 `payment_id`(구독 직접 결제) **또는** `order_item_id`(강의 플랫폼 주문) 중 정확히 하나(`CHECK num_nonnulls(...)=1`). item 에 `fee_krw`·`settle_base_krw`(매출 기여분, 환불은 음수) 추가. settlement 에 `gross_krw·refund_krw·fee_krw·net_sales_krw·fee_rate_bp·tax_type·tax_rate_bp·tax_krw·payout_krw` 추가. 산식: 매출=결제−환불−수수료, 정산금액=매출×비율, 지급액=정산금액−세금액. ★`payments` 중 `order_id IS NOT NULL` 은 정산 원천에서 제외(order_items 와 이중 계상).
- **orders.paid_at** (feat-8-031): 결제 완료(무통장은 입금 확인) 시각. `markOrderPaidAndFulfill` 첫 전이에 기록. ★강사·도서 정산의 **월 귀속 기준** — `created_at` 을 쓰면 월말 주문·익월 입금 확인 건이 이미 확정된 달에 묻혀 영영 빠진다. 또한 `markOrderRefundedAndRevoke`(주문 전체 환불)가 종전엔 `order_items.refunded_at` 을 남기지 않아 order_items 기준 집계에서 전체 환불이 안 보였다 — 2026-09-12 수정·소급 기록.
- **order_status_logs** (★2026-09-17 feat-11-014 Q1, `scripts/sql/20260917_q1_order_status_logs.sql`): 관리자 주문 상태 셀렉트의 이력 — `log_id·order_id(FK cascade)·from_status·to_status·reason(not null)·actor_id(profiles)·created_at`. append-only. **서버 액션이 adminClient 로 직접 insert**(트리거·GUC 방식 아님 — `refund_status_logs` 의 GUC 는 service_role 쓰기에서 actor 가 빈다). 보관은 status 전이가 아니지만 같은 표에 `to_status='archived'` 로 남긴다. RLS: staff select. `audit_logs` 에 `order.status_change` 미러.
- **orders.archived_at** (★2026-09-17 feat-11-014 A2 「삭제」= 보관): timestamptz null. 값 있음 = 기본 목록에서 숨김(「보관함」 필터로 열람). 서버가 결제 전 상태(draft/attempted/pending_payment/expired/cancelled/failed)만 허용 — 결제·환불 주문은 거부. 행·FK·통계 무변경(삭제가 아니다).
- **lecture_schedules** (feat-12 현장강의 일정, 공개 달력 `/lecture/schedule`): `subject_code`(★2026-09-17 CHECK 7값 = LMS 과목 6 + `elective2`「2차 선택」 — 일정·캘린더 전용 묶음, `books_subject_code_check` 6값과 다름)·`subject_label`(코드에서 서버가 생성)·`exam_round`(★2026-09-17 `scripts/sql/20260917_schedule_exam_round.sql`: NULL|round1|round2 = 구분 없음/1차/2차, 캘린더 구분 필터)·`format`(offline|live|video — 2026-09-17 부터 폼은 offline 만 쓰고 공개 화면은 offline 만 표시)·start_date·day_label·time_label·capacity/enrolled·status(open|soon|waitlist|closed)·plan_code·published·deleted_at. SSOT `app/features/landing/lib/schedule-taxonomy.ts`.
- **refund_requests** (feat-8-029 P3, ★2026-09-17 feat-11-014 Q3 에서 코드 경로 제거 — 표는 남김(운영 0건), 후속 DDL 로 drop 예정): 사용자 개시 환불요청. order_item_id·user_id·reason·status(pending/approved/rejected)·resolved_by/at·resolve_note·refunded_krw. 항목당 대기중 1건 부분 유니크 인덱스. RLS: 학생 insert 본인·select 본인+staff, 승인/거절 UPDATE 는 service_role(adminClient). 승인=refundOrderItem(토스 부분취소·회수·CS 미러) 실행 후 approved 기록.
