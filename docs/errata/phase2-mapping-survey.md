...
**cases** (교재별 텍스트 라벨 `comment_source`: 리담특허법 판례 **[제9판]** · 리담상표법 판례 **[제16판]** · 리담 디자인보호법 판례):

| 법률 | 전체 | book_sections | source_seq | comment_source |
|---|---|---|---|---|
| patent | 383 | 0 | 371 (97%) | 215 (56%) |
| trademark | 356 | **356 (100%)** | 356 | 132 |
| design | 62 | 0 | 62 (100%) | 62 |

특허의 comment_source 무 168건도 대부분 source_seq는 있음(156/168) — 라벨은 평석 보유분에만 기입된 것. 최근 추가 11건은 최신판례(교재 밖 신규)로 정상.
...
| 문제 번호 | ✅ `problem_number` 기출·예상 전량 (예외: ai_draft 7건·자과 예상 8건 없음) | ★단, 의미가 "책 통번호"가 아니라 **체계도 노드 내 순번**(워크북 section 기준 재배치 — 특허·상표 완료). 책 전체 순번은 체계도 트리순으로 파생 재계산(`attachProblemOverallNo` 기존 로직) |
| 실제 시험 문항번호 | `exam_number` 561건(past_exam 517+variant 44, 특허 위주) + `year` 기출 전량 | 기출문제집이 연도·회차 편제라면 이 축으로도 책 내 위치 서술 가능 |
...
5. **상표 워크북 문서 대장 미등록** — 상표 객관식은 문제편 기반 재배치까지 끝났는데 `problem_source_docs`에 상표 워크북(판 미상 — 등록 시 확인) 행이 없어 신규 14건이 미연결이다. **권고: Phase 2에서 문서 행 추가 + 14건(및 특허 3건) 연결 백필**(소량·기계적).
