...
| feat-7-019 | 반/기수 통계 모니터링 (`/admin/cohorts/:id/stats`) — feat-7-010 진도(학생별 행)와 분리된 cohort 평균/분포 종합 화면. 평균 KPI(평균 정답률·평균 시도·평균 조문 열람·최근 7일 활동 학생수). 정답률 5구간 분포(80+/60-79/40-59/20-39/0-19) 막대. 5과목 평균 표(평균 시도·평균 정답률·평균 조문 열람). 상/하위 5명 카드(정답률 기준). 신규 함수 `getCohortAggregateStats` (admin client RLS 우회 — `listCohortProgressSummary` + 멤버 과목별 attempts/sessions 집계). cohort-detail 과 progress 양쪽에 진입 링크. | P1 | ✅ |
