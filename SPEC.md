...
| feat-3-209 | **학습보조 기간 필터** — `/study/*` 5개 페이지(하이라이트·포스트잇·메모·즐겨찾기·오답노트) 의 `FilterBar` 에 KST 자정 기준 "오늘·7일·30일·전체" preset chip 추가. `study-aids-list.tsx` 의 공용 `RangePresetGroup` + `inRangePreset(iso, preset)` 헬퍼(KST 자정 ms 계산). 각 페이지가 본문에 적합한 시각 필드로 필터 — highlights=`createdAt`, notes/comments/bookmarks=`updatedAt`, wrong-note=`lastAttemptedAt`("최근 시도" 라벨). 초기화 동작·hasActive 판정에 통합. | P2 | ✅ |
| **5.3.3 객관식 문제** | | | |
