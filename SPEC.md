...
| feat-11-011 | **관리자 메뉴 재편·접근불가 6화면·결제/연장 안정화(260903 요청서).** 설계=`docs/features/feat-11-011-admin-ia-and-payment-stabilization-260903.md`. §2 접근불가 6화면 원인 규명 완료(코드 버그 아님 — duty 미배정·원장전용 가드 + 403 이 일반 오류로 렌더). P0 권한차단 화면 분리·사이드바 duty 필터·테스트계정 위생 / P1 결제시도 상태 분리(pending_payment 24건 적체)·TTL 만료 / P2 표시 계층(title_snapshot·course_extension 한글화·도서명) / P3 관리자 2모드 메뉴 재편 / P4 목록 상태 유지 / P5 대시보드 / P6 쿠폰·포인트 / P7 잔여 델타. **설계 승인 대기(D1~D10).** | P1 | 🔲 |
