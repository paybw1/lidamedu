...
| feat-3-209 | **학습보조 기간 필터** — `/study/*` 5개 페이지(하이라이트·포스트잇·메모·즐겨찾기·오답노트) + `/me/ox-sessions` + `/me/ox-wrong-note` + AI Q&A 대화 사이드바(`/ai`) + 학습 통계(`/study/stats`) 에 KST 자정 기준 "오늘·7일·30일·전체" preset 추가. `study-aids-list.tsx` 의 공용 `RangePresetGroup` + `inRangePreset(iso, preset)` 헬퍼(KST 자정 ms 계산). 각 페이지가 본문에 적합한 시각 필드로 client-side filter — highlights=`createdAt`, notes/comments/bookmarks=`updatedAt`, wrong-note=`lastAttemptedAt`("최근 시도"), my-ox-sessions=`completedAt`/`startedAt`("기간"), my-ox-wrong-note=`lastAttemptAt`("최근 시도", 필터 변경 시 runner 리셋), `/ai` 대화 list=`updatedAt`(사이드바 grid 4 col). **stats** 는 URL `?range=` search param + loader 가 시계열 query 인자에 매핑(today=1·7d=7·30d=30·all=84일 / accuracy weeks: 1·1·4·12 / pass days: 1·7·30·30). 누적 stat 은 항상 전체 — 헤더 hint 로 적용 범위 안내. | P2 | ✅ |
| **5.3.3 객관식 문제** | | | |
