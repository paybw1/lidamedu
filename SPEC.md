...
| feat-11-006 | **콘텐츠 라이브러리 + 강의그룹(콜러스 운영 고도화).** 설계=`docs/features/강의플랫폼-추가설계-방향.md`. Phase 1 콘텐츠 라이브러리(`video_contents`+`content_groups`, lesson_videos.content_id 재배선·하위호환 백필, 진도율 인정 기준 콘텐츠 컬럼화) / Phase 2 통합 탭 등록 UX(유형·카테고리·복수강사·강의복사·변경이력) / Phase 3 통합 HTML 에디터(소스 우선 하이브리드) / Phase 4 수강평·교재평·도서 무료배송 임계·과세면세. **Phase 1 Stage 1(스키마+백필) 적용 완료**(2026-07-20, 11 videos→4 contents 이관·전량 링크). 잔여: 콜러스 동기화 job·콘텐츠관리 화면·등록 재배선. | P1 | 🟡 |
