...
**feat-9-004 진행 상황 (🟡)**: DB 마이그레이션(`ai_conversations` + `ai_messages` + `ai_message_role` enum + RLS 본인만 R/W + soft delete + 메시지 insert 시 부모 updated_at 트리거). 대화 CRUD(`conversations.server.ts` — listMyConversations 미리보기·count 포함 / getConversationWithMessages / createConversation / appendUserMessage / appendAssistantMessage / softDeleteConversation / autoTitleFromQuestion / setMessageFeedback) + 멀티턴 빌더(`buildMultiturnMessages` — AI_QNA_MULTITURN_LIMIT=4). `answer.server.ts` 시그니처 `messages[]` 받도록 변경(멀티턴 지원). 실사용 SSE `POST /api/ai-qna/ask` — 인증만, conversation_id 또는 새 대화 자동 생성, anchor·lawCodes 옵션, user 메시지 검색 전 저장 + assistant 메시지 done 시점 저장. 채팅 화면 `/ai` (`screens/ai-chat.tsx`) — 좌측 대화 list / 우측 메시지·출처 카드·입력창, 추천 질문 빈 상태, SSE 클라이언트(fetch+ReadableStream), 피드백 👍/👎(action), soft delete 버튼. navigation-bar 학습보조 dropdown 에 "AI Q&A (베타)" 진입점. **잔여**: ① ~~뷰어(조문/판례/문제) 우측 패널에 "AI에게 묻기" 진입점 + 앵커 전달~~ ✅ (2026-05-21 — AskAiButton 공용 컴포넌트, 조문/판례/문제 viewer 헤더 3곳 모두 추가, 시드 질문 자동) ② 대시보드 "최근 AI 대화" 카드 ③ ~~markdown 렌더링 (현재 whitespace-pre-wrap)~~ ✅ (2026-05-21 — MarkdownView 적용, assistant 본문만. user/스트리밍 중은 pre-wrap 유지) ④ 신규 대화 자동 제목을 LLM 요약으로(현재는 질문 60자 truncate).

**feat-9-005 진행 상황 (🟡)**: ai_messages 에 `feedback_note text` + `feedback_at timestamptz` 컬럼 추가 + partial 인덱스(`feedback = -1`). staff RLS 정책 — `feedback IS NOT NULL` 메시지/부모 대화만 select 가능(사용자 학습 데이터 무차별 노출 차단). `setMessageFeedback` 가 note 옵션 받음(👎 시 1000자 자유 사유). `/ai` 채팅 화면 👎 클릭 시 인라인 textarea 노출(blur/명시 버튼 저장). RPC `ai_qna_daily_metrics(p_days=30)` / `ai_qna_total_metrics()` security definer + staff 체크 — 일별 응답·👍·👎·거절(가드레일 ③⑤ 정형 문장 매칭)·input/output 토큰, 누적 응답·사용자·토큰·피드백. 운영자 화면 2종: `/admin/ai-qna/feedback`(👎 큐, 질문·답변·사유·출처·강사 Q&A 에스컬레이션 링크), `/admin/ai-qna/metrics`(누적·최근 7일 카드 + 일별 표 + 추정 비용 ₩1,400/$ 환산). **잔여**: ① eval 데이터셋 테이블(`ai_eval_items` — 강사 라벨 골드 정답) + 자동 평가 cron(v1.1) ② 정형 거절 문장 매칭 → enum `refusal_kind` 컬럼으로 정교화 ③ 사용자별 월별 사용량 export(정산 데이터) ④ 강사 검토 완료 표시 컬럼(review_status).
