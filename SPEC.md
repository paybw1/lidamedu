...
| feat-3-206 | **판례 본문 밑줄 마커 반영** — 리담특허법판례 hwpx 원본의 underline 영역(`hh:charPr` 의 `BOTTOM` underline)을 `<u>...</u>` 인라인 마커로 추출해 `cases.summary_items / summary_body_md / reasoning_md / comment_body_md` 에 반영. 뷰어 `Prose` 가 `renderWithUnderline` 으로 React `<u>` element 렌더(다른 HTML 마크업은 들어오지 않는 전제). `search_tsv` generated 식·AI Q&A `chunker.normalizeBody` 는 `<u>` strip. 적용 도구: `scripts/precedents/hwpx-to-text-underline.mjs` + `parse-hwp-text.mjs`(seq optional 보강) + `apply-underlines.mjs`. 결과: 특허법 404 case 중 331건(82%) 에 밑줄 마커 입력. | P2 | ✅ |
...
| feat-3-209 | **학습보조 기간 필터 (v2 + 사용자 지정)** — `/study/*` 5개 페이지(하이라이트·포스트잇·메모·즐겨찾기·오답노트) + `/me/ox-sessions` + `/me/ox-wrong-note` + AI Q&A 대화 사이드바(`/ai`) + 학습 통계(`/study/stats`) 에 KST 자정 기준 "오늘·7일·30일·전체" preset + **사용자 지정(from~to 달력)** 추가. 공용 헬퍼 `study-aids-list.tsx` 의 `RangePresetGroup` / `RangeSelectionGroup` / `inRangePreset` / `inRangeSelection` + `ALL_RANGE_SELECTION`/`isRangeSelectionAll`. 학습보조 5개는 RangeSelectionGroup(preset 4 + 사용자 지정 popover, native `<input type="date">`) 으로 마이그레이션. **stats v2**: URL `?range=&from=&to=` + loader 가 (1) 시계열 query 인자 매핑(daily/accuracy/pass) (2) **누적 통계 since 인자 적용** — `getDashboardKpis`, `getStudyAidCounts`, `getUserSubjectiveStats`, `getUserBlankStats`, `getUserAutoBlankStats`, `getUserRecitationStats`, `getWeakAreas`, `listWrongAttempts`, `listOxWrongAttempts` 가 since 옵션 받음. 진도(`getOverallProgress`/`getAllSubjectsProgress`/`getAllScienceSubjectsProgress`) 와 visited 기반 stat(`getArticleStudyStats`/`getCaseStudyStats`) 는 누적 유지. | P2 | ✅ |
| **5.3.3 객관식 문제** | | | |
