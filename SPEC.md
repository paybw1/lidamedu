...
| feat-3-604 | **판 대조 도구** — 인쇄된 책(PDF) ↔ 개정중 원고(HWPX)를 글자 단위로 맞춰 추록·정오표 후보를 뽑는다. `scripts/errata/compare-book-editions.mjs` + 규칙 모듈 `lib/book-diff.mjs`. 본문/각주/표 칸/도형을 갈라 대조(본문 99.8%·각주 98.9%·표 97.9% 자동 정합), 쪽수는 PDF 러닝헤더에서 취함. 산출 changes.csv/html/json. **S2 완료** — ① 짝짓기 뒤 정렬 무관 재확인(확실/일부/이동 3등급, 자리만 옮긴 76건 접힘) ② 표·그림 목차 대조(양쪽 246개 → 없어짐 1·새로 생김 1·번호밀림 2) ③ 표·도해 후보 쪽을 mupdf 로 렌더해 나란히(구판 그림 ↔ 신판 글자). 볼 것 130건 / 86쪽. **S3 완료** — `book_diff_candidates` 검수함(적재=service_role 스크립트, 판정=staff, 재적재해도 판정 보존) + `/admin/book-diff` 검수 화면 + `fn_publish_book_errata`(원장 메타 행 → 기존 `fn_publish_errata` → 시트 PDF·`/study/errata`). `content_type='theory'` 첫 사용처. 운영 적재 130건(전부 미판정) · 판본 「리담특허법 제25판」 등록. 발행은 원장이 화면에서 누른다. | P2 | ✅ |
...
| feat-2-036 | **주관식 모범답안 연습 모드 (목차 연습 · 내용 연습)** — 판례 도식 S6(feat-2-035 §10)의 "가리고 복원"을 2차 주관식 모범답안에 적용. 원장 요청(2026-09-04)대로 **① 목차 연습**(발문만 보고 목차를 세운다)과 **② 내용 연습**(목차는 보이고 잎 칸 본문이 빈칸)으로 나눈다 — 목차와 내용은 틀리는 방식이 다르므로 섞으면 무엇을 고칠지 알 수 없다. ★기존 3단계 훈련(feat-2-032 S5)과 중복 아님: 그쪽은 **백지에서** 스스로 세우고 AI 채점, 이쪽은 **모범답안을 복원**하고 즉답·무료 채점 — 백지 훈련 전 단계의 사다리. 전수 실측 184건: `##` 없는 답안 0건, 중앙 설문 3·중목차 11·소목차 9·3,568자 → **설문(`##`) 단위로 끊는다**(전체는 20칸이라 아무도 안 끝낸다). 채점은 `answer-match.ts` 공용(새로 만들지 않음), 임계값 0.65/0.35 그대로 — **짧은 제목에도 통하는지 구현 전 실측**: 뼈대 같은 목차 0.70 / 큰 뼈대만 0.19 / 다른 논점 0.04 (도식과 같은 자리, `outline-practice-probe.test.ts`). 순서는 별개 신호로만 표시하고 감점 없음(실측 없는 감점 금지). 저장은 v1 localStorage — `user_subjective_attempts` 에 넣으면 3단계 훈련 KPI 가 부풀어 완주율이 뜻을 잃는다. ★**원장 결정 3건 반영(2026-09-04)**: ①**staff 전용 유지** — 주관식 뷰어 게이트 2겹(`problem-viewer.tsx` staff 리다이렉트 + `redactSubjectiveAnswer` 서버 가림)을 그대로 둔다(한 겹만 열면 학생에게 빈 연습 화면이 뜬다). ②임계값은 그대로 두고 **안내 문구**를 붙였다(법령·판례가 쓰는 말이 담겼는지로 채점 — 풀어 쓰면 낮게 나온다). ③**단서 없는 칸은 묻지 않는다** — 본문 없이 하위를 묶기만 하는 자리(「2. 甲의 주장」)는 목차 연습에서도 제목을 그냥 보여 준다(`blanksOf` 가 두 모드 모두 `block.leaves`, 두 연습이 완전한 거울). 모범답안 카드 안 「읽기/목차 연습/내용 연습」 토글, 비제어 입력(iPad IME)+localStorage 초안. 전수 감사 184건 글자 어긋남 0. 설계 `docs/features/feat-2-036-essay-answer-practice.md`. | P2 | ✅ |
| feat-2-037 | **도해특허법 빈칸 학습 모드 (유형 1·2·3)** — 도해 유닛(94개)의 표·본문 낱말에 빈칸을 뚫되, **그 단원의 기출·정오문제에서 실제로 논의된 말만** 고른다. 원장 요청(2026-09-04). **용도는 1차 객관식** — 2차용 학습모드(판례 도식 feat-2-035 S6 · 모범답안 feat-2-036)와 목적이 달라 채점도 서술 커버리지가 아니라 **낱말 정오 판정**(조문 빈칸과 동일). 유형 1=기출 유래 · 유형 2=정오(OX) 유래 · 유형 3=합집합(**원장 확인 2026-09-04**). **조문 원문 박스는 제외**(조문 빈칸이 이미 있다) — ★블록 JSON 에는 남아 있고 렌더에서 갈아끼우는 것도 유닛당 첫 박스 하나뿐이라 **판정은 블록에서 직접** 한다(안 그러면 「조약의 효력」에서 교재 조문이 빈칸이 된다). 소제목(`h`)·도해 이미지도 제외. **구현 전 전수 실측**(`scripts/dohae/probe-blank-terms.ts`, 저장 없음): 말은 **문제 쪽에서 모으고 유닛 본문에서 찾는다**(도해 표 칸은 조사가 없어 `nounStem` 이 유닛 본문에선 아무것도 못 잡는다). 거르기 3종 — ①조사가 실제로 벗겨진 체언만(안 그러면 「공지된」·「받을」이 상위권, 「진보성/진보성이/진보성을」로 갈라짐) ②**코퍼스에 홀로 쓰인 적 있는 말만**(★「선출원주의」→`의` 를 떼면 「선출원주」. 「적용받」·「소급되」도 같이 걸림) ③문서빈도 8%(2글자는 2%) 상한(「발명」·「경우」는 답이 뻔하다). 결과 = 88/94 유닛에서 생성, 평균 30개(0개 6·1~4개 5), 실제 예: 최후거절이유통지·보정각하결정·신규사항·기재불비(t25) / 정정심판·확인대상발명·특허취소신청·권리범위확인심판(r6-2). ★**유형1·2 어휘는 많이 겹친다**(기출만 127·정오만 1,016·둘다 1,749 — OX 지문이 기출 선지에서 나오기 때문). 그래도 **유형1은 기출에서 온 말만·유형2는 OX 에서 온 말만** 뚫어 실측 40.0 / 54.5 / 56.0칸으로 갈린다. 저장 = `dohae_blank_terms`(unit_id·term·from_exam/from_ox·근거 건수·`excluded_at`) — ★**좌표는 저장하지 않는다**(조문 빈칸은 재직렬화 한 번에 96세트 좌표가 유실됐다; 도해 본문은 운영자 편집·재파싱 둘 다로 바뀐다). 위치는 렌더 시 본문에서 찾는다. 입력은 판례 빈칸 골격 + **비제어**(채점이 「맞춰보기」 한 번이라 키마다 판정할 일이 없고, state 로 되쓰면 타이핑마다 표가 다시 그려진다) + pointerdown 선행 flush · Enter=blur→rAF focus · **입력 칸 key=`termId:start`**(★번호로 주면 유형을 바꿔도 React 가 같은 자리 칸을 재사용해 앞 유형에서 친 답이 다른 말의 칸에 남는다). V2 contenteditable 은 조문 전용이라 미이식 + **iPad 실기 확인이 완료 조건**. 빈칸 모드에서 하이라이트·포스트잇은 **렌더만 멈춘다**(저장 오프셋 불변 — 입력 칸이 글 흐름에 들어가면 어긋난 자리에 그어진다). ★빈칸 모드 진입 시 접힌 절을 **한 번** 편다(안 그러면 입력 칸이 하나도 안 보이는데 「맞춰보기」가 "0/0 정답"을 낸다). 유출방지 4겹 유지. 기록·SRS 합류는 별건(`user_blank_srs` 는 `set_id` 매인 구조, `user_problem_attempts` 는 학습현황 신호 오염). 공개는 **staff 선출시 → 검수 후 학생**. **S1~S6 완료** — ★**S6(2026-09-05) 밀도 재조정**: 원장 「양이 너무 적다」 → 상한을 잡던 것은 배치가 아니라 **말의 수**였다(글조각 유닛당 54.7개인데 유형당 12개 말로만 채웠다). **밀도 = 한 줄에 하나**(`LINE_CHARS=40`, 글조각 몫 = ceil(글자수/40)) · **같은 말 5회까지**(다 풀면 t20 「종업원」 67회) · **말 수 상한 폐지** · 적재도 자르지 않음(headroom=∞). ★상한에 걸린 말은 건너뛰고 자리를 다음 말에게 넘긴다(멈추면 그 글조각이 통째로 빈칸 없이 지나간다). 결과 적재 1,802→**2,830행**(삭제 0·뺀 말 보존) · 배치 18.1→**56.0칸**(유형1 40.0·유형2 54.5) · 단위 19건 · 감사 ③「한 칸에 두 번」 0건 유지. 남음 = 원장 검수(①답이 화면에 있다 154건 · ②두 글자 일반어 727건 — 목록 확정되면 추출 상수로) · 학생 공개 판단. **S7(2026-09-11) 표 칸 가리기(정리비교표식)** — 목차칸(교재 음영 라벨)을 누르면 그 줄·그 칸이 가려지는 읽기 화면의 **「칸 가리기」 모드**(토글 · 낱말 빈칸과 배타 · 저장·DB 변경 없음 · **2026-09-11 학생 공개**, 낱말 빈칸은 staff 전용 유지). ★늘 켜 두면 더블클릭 낱말 선택·선택 해제 클릭과 부딪혀 모드로 묶었다(검토 지적). 규칙은 렌더 때 `shade`·격자 좌표로 센다 — ★정리비교표 규칙(첫 줄·첫 열만)을 그대로 옮기면 **둘째 열의 작은 라벨 1,170칸이 전부 죽은 손잡이**가 된다 → 머리띠(전부 음영인 줄)=아래 세로줄 · 그 밖의 음영=오른쪽 가로줄 · 모서리=표 전체 · 속표는 바깥 칸의 자식. 전폭 각주 줄(「* 단, …」)은 행 목차 판정·세로줄 대상에서 제외(실데이터 9표가 이 줄로 무너져 있었다). 실측 칸 2,555 · 손잡이 2,481 · 붙는 유닛 84/94(`scripts/dohae/probe-cell-blanks.ts`). E2E `e2e/admin/dohae-cell-blanks.spec.ts`. 설계 `docs/features/feat-2-037-dohae-blank-practice.md` §13. | P2 | 🟡 |
| feat-2-038 | **정리비교표 화면 적재 (특허법)** — 교재 부록 정리비교표 13쪽을 체계도 **대분류**에 붙여 조문 탭 「정리」 화면에서 읽게 한다. 원장 지시(2026-09-09): **이미지 금지** — 나중에 빈칸 학습(feat-2-037 계열)을 걸어야 하므로 모든 상자·칸이 선택 가능한 글자여야 한다. 표 7쪽(4~9p·12p)은 병합(colspan/rowspan)까지 교재 그대로 옮겼고(584칸 대조 0 다름), 도형 5쪽(2·3·10·11·13p)은 절대배치 div + SVG 선으로 그렸다. ★도해의 연결 관계는 좌표가 아니라 **강의노트 PPTX 의 stCxn/endCxn** 이 정답(저장된 선 좌표는 낡음). ★목차 칸 한 글자 세로쓰기는 **내용 열 5개 이상인 쪽만**(9·12p 에 걸면 아홉 줄이 선다). 저장=`systematic_digests`(law_code+page unique, body_html + `.digest-doc.dpN` 로 좁힌 css). ★한 단원에 두 쪽이 붙는 곳(01 총칙=2p+3p, 06 심판=8p+9p)은 쪽 클래스로 CSS 를 갈라야 한다 — 안 그러면 두 쪽 규칙이 섞인다(`check-css-collision.mjs`). 파이프라인 `scripts/digest/{pipeline,convert.mjs,import-digests.mjs}`. **12쪽 적재·화면 연결 완료.** ★진입 = **조문 체계도 단원 화면의 도해 배지 옆 「정리」 배지 → 전체화면 팝업**(그 노드가 속한 장의 자료). 좌패널 「정리」 목차는 걷어냈다(원장 2026-09-10). 전체 체계도(2p)만 어느 단원에도 속하지 않아 조문 탭 좌패널 토글 옆 「전체 체계도」 버튼으로 연다(원장 지시 2026-09-09 — 본문 자리에 넣으면 4p 가 11px 로도 세로 1,094px 이라 한 화면에 안 들어온다). 자료는 기준 폭(`.digest-page`)에서 판이 짜여 있고, 화면에서는 **자리가 넓으면 늘려 앉히고 그래도 넘치면 통째로 축소**한다(FitPage — 글자만 줄이면 표의 가로세로 비가 교재와 달라진다). 「원본 크기」 전환 있음. **2026-09-11 학생 공개(원장 결정)** — 도해와 같은 유출방지 다섯 겹(워터마크·복사차단·인쇄숨김·첫 열람 고지·열람 로그 `systematic_digest_views`)을 팝업에 붙인 뒤 read 정책(`systematic_digests_read`, 로그인 사용자)을 열었다. 이용 가이드(학습과목 「정리비교표 — 단원을 한 장으로 보고 빈칸으로 외우기」)·공지 발행. | P2 | ✅ |
| feat-2-039 | **상표법 2차 주관식 모범답안·채점기준(원장 지시 2026-09-17).** 설계=`docs/features/feat-2-039-trademark-essay-answers-260917.md`. 특허 작성 원칙 그대로(3단계·절 통독·사건번호 4곳 확인·현행법·넘버링/표기/분량 규칙) + 상표 고유(자기법 접두 삭제·2016 전부개정 대응·지식재산처). 근거 = 리담상표법 20판(로컬 코퍼스 912청크, `tm-book-read.mjs`) · 판례 16판(DB cases 359, `tm-case-find.mjs`) · 2차도서 4권(참고 전용, `tm-2cha-find.mjs`) · 채점위원 채점평. 범위 A군 40문항(모범답안 없음, AI 생성 스탬프)·B군 28문항(7월 생성분 재작성 여부 원장 확인). 절차: 파일럿 1문항(2015-1) → 하드스톱 → 워크플로(작성→적대 검증→수정)·스윕·audit → 하드스톱 → 반영. **2026-09-19 현재 68/68 초안 완료**(작성·적대 검증·수정 전 단계 통과, 전수 스윕 FAIL 0·평균 172자/점), 검토판 v7 게시, **DB 미반영 — 하드스톱 #2(원장 표본 검토) 대기.** 사람 확인 요청 누계 669건(문항당 중앙값 10건)·원장 보고용 데이터 이상 목록은 설계 문서 「5. 진행 기록」 참조. | P1 | 🟡 |
...
| feat-7-049 | **본문 찾아 고치기(운영자)** — `/admin/tools/find-replace`, staff 전용. 같은 오기가 여러 콘텐츠에 흩어져 있을 때(판례해설 인용 호수 밀림·저자명 오기·조문 번호 오타) 스크립트 없이 찾고 고친다. 대상=판례 본문(`summary_items`·`book_sections` 포함)·판례 배치 서술·참고문헌·문제 해설. **제외**=법령 조문(불변)·판례 식별 필드·2차 모범답안/채점기준(별도 감사 파이프라인)·문제 발문·정규식. 흐름=찾기(`find_content_matches` RPC 가 DB 안에서 스캔) → 필드별 미리보기(등장 횟수·앞뒤 맥락) → 선택 적용(적용 시점 재확인, 상한 200) → batch 단위 되돌리기(`content_edit_logs`, compare-and-swap). 상세: `docs/features/feat-7-049-content-find-replace.md`. | P1 | ✅ |
...
| feat-8-031 | **강사 정산현황 (원천 확장·수수료/세금·본인 팝업)** — feat-8-029 후속. ①정산 원천에 강의 플랫폼 주문(`order_items`, item_type=plan/course_extension)을 추가하고 `payments` 중 `order_id` 있는 건은 제외(이중계상). 결제액=단가×수량−주문 쿠폰할인 안분(배송비 제외), 환불도 같은 비율로 축소. ★`orders.paid_at` 신설(월 귀속 기준)·주문 전체환불이 `order_items.refunded_at` 을 안 남기던 구멍 수정. ②파라미터 `settlement_settings`(PG 수수료율, 기본 0=미설정)·`instructor_settlement_profiles`(강사별 세금유형, 기본 개인 3.3%) — 정산서에 스냅샷. ③산식: 매출=결제−환불−수수료, 정산금액=매출×정산비율, 지급액=정산금액−세금액(`settlement-engine.ts` 순수함수 SSOT, 단위테스트 14). ④강사 귀속=`plan_courses`→`course_series.instructor_id`(복수면 강의 수 안분, 규칙 없으면 missingRule 보고). ⑤상단 계정 아이콘 팝업 + `/lecture/settlements` + `/api/lecture/settlement`(CSV) — 확정·지급 달은 스냅샷, 그 외는 「집계 중·예상」 실시간 계산. 소유자 서버 강제(IDOR 차단)·수강생명 마스킹·확정/지급 알림. 상세: `docs/features/feat-8-031-instructor-settlement-status.md` | P1 | ✅ |
...
| feat-11-011 | **관리자 메뉴 재편·접근불가 6화면·결제/연장 안정화·포인트(260903 요청서).** 설계=`docs/features/feat-11-011-admin-ia-and-payment-stabilization-260903.md`. **P0~P7 전 단계 배포 완료(2026-09-03)**. P0 권한차단 화면 분리·사이드바 duty 필터·테스트계정 26명 강등(원장 16→1) / P1 결제시도(attempted) 분리·TTL 만료·적체 24건 정리·toss_payment_key UNIQUE / P2 order_items.title_snapshot + 표시 SSOT(내부코드 노출 0) / P3 사이드바 2모드(학습 관리 | 강의·판매 관리, 요청서 10개 업무군) / P4 목록 필터 유지(URL SSOT + sessionStorage 힌트) / P5 운영 대시보드 /admin/dashboard / P6 포인트 정책 13종 + 쿠폰 교환 / P7 최대기기수 입력란 삭제·안내문 제거·회차 삭제 영향표시. 추가: 운영자 구독 환불(/admin/payments). **잔여**: 자주 쓰는·최근 사용 메뉴, breadcrumb 통일, 중복 URL redirect 정리, 브라우저 왕복 검증. | P1 | ✅ |
| feat-11-012 | **강의 플랫폼 오픈 전 정비(260913 체크리스트).** 설계=`docs/features/feat-11-012-lecture-open-readiness-260913.md`. 조사 8축 160건 → 중복 정리 60여 건 → 묶음 12개. **성격**: 기능 추가가 아니라 「이미 만들어 둔 것이 화면에 닿게」 — 공용 부품을 만들거나 되찾아 N건을 한 번에 닫는다. P0 게이트 전제(★학생 게이트 4종이 /lecture 에 안 걸림 — private.layout 미중첩) / P1 검색 노출(사이트맵 7건 전부 이중 슬래시·강의 경로 0건, canonical 0곳, lang=en, next 무시) / P2 폰(모바일 탭 20개 평탄화 한 줄·헤더 최소폭 480→374·구매바 셸·스와이프 훅) / P3 첫 화면(배너 0건이면 h1 0개 → 소개 히어로 폴백·하드코딩 숫자 실측화) / P4 결정 정보(강사·회차 공급자 확장·본문 채우면 포함강의 사라지는 분기 버그·PriceTag) / P5 결제(세트 한 줄·바로구매 복귀 분기·결제결과 단일 리더·금액 권위 서버·쿠폰 KST·포인트 잔액 DB) / P6 산 다음(재생 판정 단일화·하트비트 소진 가드·EmptyState 되찾기·표기 SSOT) / P7 잔손질(확인 다이얼로그 2파일·공용 페이저). ★원장 결정 대기: D1 종이책 / D2 무통장 / D3 게이트 범위 / D4 도메인 단일화 / D9 학생 게이트 적용 구간 / D10 ENFORCE_DEVICE(충돌한 두 결정) / D11 후기 노출 / D12 진도율 분자. | P7 | 🟡 |
| feat-11-013 | **강의상품 과정유형 · 환불관리(260914 요청서).** 설계=`docs/features/feat-11-013-course-types-and-refunds-260914.md`. PART A 과정 유형 6종(온라인상시/온라인정규/현장/온·오프혼합/정규패키지/상시패키지) 선택 + 유형별 조건부 입력 / PART B 관리자 수동 환불(토스 상점관리자 직접 취소) + 환불규정 자동계산. ★감사 결과: 강의↔판매상품 M:N 분리와 `plan_policies` 수강정책 20여 칸은 **이미 있음**. 없는 것은 ①과정 유형 축 ②현장강의(정원이 수기 표시값, 판매 제한 아님) ③패키지의 강의별 정책(`plan_courses` 가 2컬럼 조인) ④**결제 당시 스냅샷 전부**(정상가·수강기간·예정회차·쿠폰 항목배분·실결제액). ★★요청서와 무관하게 **지금 틀린 것 4건**: 쿠폰 주문 환불액이 할인 전 금액이라 토스 거절/과환불 · 웹훅 PARTIAL_CANCELED 가 수강권을 안 건드림(요청서 모델의 전제) · 화면 「N일 수강」과 실제 기간이 다른 컬럼 · 재구매 시 수강권 중복 생성. ★전제 부재: 포인트 결제 기능 없음(Q=0) · 강의자료 이용이력 로그 없음(소급 불가) · 쿠폰 복원 플래그 없음. **진행(2026-09-14)**: 원장 지시 「P0+P9+P1 먼저」 — ①환불액 실결제액 기준 ②웹훅 PARTIAL_CANCELED 미반영 감지·CS 기록 ③수강기간 권위를 plan_policies 로(고정 종료일 상품은 「~까지」 표시) ④수동 부여 중복 차단, `order_items` 스냅샷 8칸 + `planned_sessions`, `material_access_logs` 신설·다운로드 기록. **진행(2026-09-15)**: P6 환불관리 — 정산 평면 교정(P0-① 잠복 결함) / 모델 3종 + 12상태 기계 / 확정 커밋 RPC + 쿠폰 복원 모델 / 토스 웹훅 소유권 정리 / 관리자 접수·처리 화면 / **토스 자동취소 제거(D9 — 단, 주문 없는 구독 결제는 제외: 막으면 학습 플랫폼 환불 수단이 사라진다)** / 학생 셀프 환불 2경로 제거 + **환불정책 제2조 2항·제3조 1항·제5조 1항 개정**(원장 승인 2026-09-15 — 권리는 유지하고 실행 주체만 「신청 → 회사 처리」로). ★**미착수**: 계산 엔진(P7)·후속처리 고도화(P8). **진행(2026-09-15 P7-핸드오프)**: ★③**부분 금액 환불이 정산에서 전액 환불로 읽히던 돈 버그** 교정(commit_refund 가 정가 전액을 쓰던 것을 `round(실환불액 × 정가 ÷ 결제귀속액)` 환산으로 — 적용 시점 환불 0건) · ①**배송비 환불 자리** 신설(`refunds.shipping_refund_krw`, 배송비 붙은 주문이 `full_done` 에 닿게). ★P7 인계(남음): 교재 수량 일부 환불은 계속 막아 둠 · 같은 항목 2회 부분청구 불가 · 무통장 계좌가 아직 리허설 값(학생 도달 불가 — 강의 게이트 뒤). ★오픈 전 블로커: **강의 플랫폼 공개 환불규정 부재**(refund-policy.mdx 는 구독 전용). **결정됨**: D10·D11·D12·D13 권고 채택 / D15 **포인트 결제 구현**(원장 지시). **2026-09-16**: D14 미채택 확정(조건부 노출은 P3) · **P2 착수**(원장 진행 OK) — 실행 설계 §3.3(결정 P2-D1~D9·실측: 강의상품 3건·`sale_status` CHECK 드리프트 `ended`→`closed` 교정), `course-format.ts` SSOT·신청내역 술어 `getPlanSaleRecords`. **P2 완료**(DDL·폼 유형 선택+잠금·목록 배지/3축 필터/기간·복사 intent·카탈로그 배지·운영 리허설 3/3·불변식 CHECK 2차 적용). **P3-a 구현(2026-09-16)** — §3.4: 폼·서버가 같은 규칙 함수 `courseFormatFormRules` 를 소비(상시=일수 고정·정규=종료일 고정·현장=정책 생략+강의 비움·회차는 상시 필수/정규·현장 null 강제), 유형 선택 전 의존 블록 미노출, 현장·혼합은 `lecture_schedules` 읽기 전용 표, 판매중+강의 0 저장 차단, `duration_days` 정책 동기화. ★운영 영향: 기존 3건 다음 저장 때 회차 입력 요구(입력 시 이후 주문 계산유형 period→single). **P3-b 구현(DDL 적용 2026-09-17)** — 정규 유형(온라인 정규·정규 패키지)만 수강 시작일·중간 신청 정책(until_end/fixed_days/closed)·판매 종료일 칸(`rules.termFields`, 그 밖은 서버 null 강제), 이행은 `term-window.ts` 순수 함수(개강 전 결제=개강일 시작 · fixed_days=신청일+N일 상한 없음 · closed=결제 409), 판매 종료 경과=카탈로그 숨김+결제 거절+상세 「판매 종료」. 다음 P4(패키지 강의별 정책)·P5(현장 이행·좌석). | P1 | 🟡 |
| feat-11-014 | **주문관리 상태 변경 셀렉트 · 결제금액 관리자 정정(260917 요청서).** 설계=`docs/features/feat-11-014-order-status-amount-260917.md`. ★조사 결론: `orders` 에는 상태 전이 트리거가 없고 지급·회수·포인트·정산 귀속은 TS 함수·`commit_refund` 가 특정 전이에 묶어 수행 → **값만 바꾸는 셀렉트는 돈 버그**(환불완료 정리 = 총매출 유지+수강권 생존 / 결제완료 = 미지급 / paid→취소 = 포인트 반환). 설계 = 셀렉트 5값 → 5개 보호된 경로(결제완료=무통장 입금확인 · 취소=비결제만 · 환불완료=환불관리 수동 종결 단축(PG 호출 0, admin) · 주문접수=무통장 재접수 · 삭제=`archived_at` 보관, 비결제만) + 사유 필수 + `order_status_logs`(서버 직접 insert, GUC 트리거 회피) + `audit_logs` 미러. 결제금액 = `orders.total_krw` 정정 + `original_total_krw` 백필 + `order_amount_adjustments` 원장(admin 전용, `payments.amount_krw` 불변, 정산·매출통계·환불캡은 안 따라옴 = 설계). ★**원장 결정 2026-09-17**: A1 채택(라벨 「입금대기」) · A2 보관 채택 · **A3 결제금액 정정 진행 안 함** · A4 안 함(PG 가상계좌·실시간이체 예정) · A5 안 함(토스 주문 셀렉트 읽기 전용). 셀렉트 = 입금대기·결제완료(무통장)·취소·환불완료·보관. **Q1~Q3 구현(2026-09-17, 로컬 커밋·푸시 대기)**: Q1 DDL 운영 적용(`order_status_logs`·`orders.archived_at`) · `allowedAdminOrderActions` SSOT(40셀 테스트) · `applyAdminOrderAction`(서버 재판정·실행 시점 가드 TTL/deposited_at/payments·이력 직접 insert·audit 미러) · 환불완료 단축 = 환불 도메인 함수 체인(접수→금액→PG기록→전이 4hop→commit, 전액만) · 화면 셀렉트+Dialog+이력+보관함 · 레거시 학생 환불요청 경로·행 「항목 환불」 제거 · PARTIAL_CANCELED 웹훅 종결 주문 ignored. dry-run 63건 정상. 한계: 구독형 수강권은 단축이 회수 안 함(안내). 운영 리허설(무통장 테스트 주문)은 원장 협의. | P1 | 🟡 |
| feat-11-015 | **패키지 등록화면 수강정책 항목 정리 · 「함께 쓰는 교재」 이미지 깨짐(260917 요청서).** 설계=`docs/features/feat-11-015-package-policy-and-book-cover-260917.md`. PART 1 ★전제 부재: 요청서의 「단과강의 수강정책」은 존재하지 않음(`courses` 는 `max_plays` 뿐, 정책은 `plan_policies` 상품 1:1). 뺄 5그룹 중 작동은 일시정지 하나(배수 dead·PC/모바일 소비처 0·다운로드 미참조·연장 tpass 거절). 폼만 숨기면 `upsertPlanPolicy` 22칸 덮어쓰기. 설계 D17 = 강의 축(재생 `courses.max_plays`) / 상품 축(수강권: 일시정지·기간) 분리 → P3-c `policyGroups` 규칙 + 서버 저장값 유지(S). ★**원장 결정 2026-09-17**: B1·B2 권고 미채택 — 「단과 강의(=강의개설 단과 상품)의 설정을 기준」: 패키지 폼에서 배수·기기·다운로드·일시정지·연장 제거, 패키지 수강권 일시정지는 구성 강의의 단과 상품 정책 상속(resolver, 폴백 없음) · B3 단과 폼도 기기·다운로드 제거 + 전역 DRM 정책 진술(D18) · B4 채택. 추가: 회원 CRM 수동 수강권 부여 「Required」= 사유 필수 스키마에 폼이 note 미전송(§6). PART 2 ★확정 버그: `lms/queries.server.ts:1128` 이 완성 URL 인 `cover_file_path` 에 버킷 프리픽스를 재적용(400 NoSuchKey, 도입 2026-07-27 부터) → `pickCover` 재사용 + object-contain + placeholder(XS). **1단계 구현(2026-09-17, 로컬 커밋·푸시 대기)**: Required 수정(사유 칸 3곳) · 교재 표지 `pickCover` 재사용+object-contain · P3-c `policyGroups`(패키지 넷 다 숨김·단과 기기만 숨김)+`applyHiddenPolicyGroups`(update 저장값 유지·create DDL 기본값) · 일시정지 resolver(`pickSingleCoursePlan`·`resolvePausePolicies`, 패키지 자기 행 폴백 차단·fail-closed) · 인계 3건(연장 진입점 게이트·종류↔유형 결합 검증(폼·복사·서버)·사유 길이 상수 SSOT). 리허설 17건 중 16건 통과(NG 1 = P3-a 회차 필수). 남음: P4. | P1 | 🟡 |
| feat-11-016 | **수동 수강권 축 혼선 · 강의 상세 목차 미노출(보완요청5 260919).** 설계=`docs/features/feat-11-016-manual-grant-axis-and-curriculum-260919.md`. ★조사 확정: 수강권은 두 축(`user_subscriptions` 구독 / `enrollments` 영상 수강권)인데 회원 상세 패널의 「수동 수강권 부여」가 **A축에만 insert** 하고 드롭다운에 강의 상품(course/tpass) 필터가 없었다(같은 기능의 /admin/subscriptions 는 이미 막혀 있어 **한쪽만 뚫림**) → "처리되었습니다"는 정직하나 테이블이 틀림, 내 강의실(enrollments만 읽음)에 미노출. 파생: A축 「한 사람 한 구독」 `auto_cancel` 이 직전 강의 부여를 취소(09-18 로그 실측). 「결제 실패 ₩180,000」은 **부여가 만든 것이 아님**(부여 경로는 orders/payments 미생성, 문구 writer 는 학생 결제 경로 `cancelPendingCheckout` 뿐, 주문이 4분 앞섬). 목차는 **렌더 코드 자체가 없음**(loader 가 lessonCount 만 집계) + 조문강의는 강의 draft·회차 2건 전량 비공개라 붙여도 0건. ★`lesson_videos` staff-only RLS 라 영상시간은 adminClient 필요. **P0 완료(2026-09-19)**: labels.ts `MANUAL_GRANT_PRODUCT_KINDS`/`isManualGrantableProductKind` SSOT → 화면 3곳 필터 + `grantManualSubscription` 서버 400 거부(화면 뚫려도 차단), 잘못 들어간 tpass 구독 1건 정리(감사로그 `ops_fix_wrong_axis`, 활성 잔여 0). 재지급은 대상이 staff 계정(재생 게이트 면제)이라 불요. 등급 영향은 `SELF_STUDY_PRODUCT_KINDS` 필터로 없었음 확인. ★**원장 결정: 공개예정 = `course_lessons.publish_at` 추가(가안)**. 남음 P1(목차 렌더·조문강의 발행·`grantEnrollment()` 단일 진입점·성공메시지)·P2(부분 unique 인덱스·이행 실패 노출·운영 경고 배지). | P1 | 🟡 |
...
| feat-12-003 | **강의 캘린더 정리·분류 축(260917 원장 지시).** 설계=`docs/features/feat-12-003-lecture-calendar-taxonomy-260917.md`. ① 형태 축(현장/실시간/영상) 제거 — 캘린더·홈 레일·상세·사이트맵은 현장(offline)만, 일정 폼 형태 칸 제거, 소개 문구 「현장·영상」(dd619e88). ② 구분(1차/2차) `lecture_schedules.exam_round` 신설(DDL 운영 적용) + 과목 7종(LMS 6 + 「2차 선택」 elective2, 일정 전용 CHECK) — 일정 폼 과목 select·구분 select, 캘린더 칩 2줄(URL SSOT, 달력 막대·목록·「가장 이른 개강」 공통 필터), 카드 메타 「1차 · 특허법 · …」. SSOT `landing/lib/schedule-taxonomy.ts`. 기존 6건 구분은 NULL(원장 지정). 배포 ✅ 2026-09-17(62e21037). | P2 | ✅ |

---

## 5.12 운영 고도화 — 사슬의 한 토막만 열린 상태 — feat-14

**목표 그림은 강의 → 학습 → 평가 → 컨설팅의 사슬이고, 지금은 가운데 한 토막(학습 플랫폼 + 종합반 파일럿)만 열려 있다.**
강의·커머스는 내년 준비로 의도적으로 잠가 둔 것이므로 그 지표를 성과로 읽지 않는다. 학습 플랫폼의 낮은 활성도 역시
부분 가동의 구조적 결과이지 결함이 아니다. 따라서 판단 기준은 **"사슬이 다 붙었을 때 버려지지 않을 것에만 지금 투자한다"**.
설계 SSOT: `docs/features/feat-14-ops-integration.md` (2026-09-01 제안, **승인 대기**).

실태(운영 DB): 처리 큐는 깨끗(Q&A 미답변 4 · 신고 미처리 3 · 문의 2 · 승인대기 2 · 등업 0) —
실제 부하는 **콘텐츠 검수**(도식 draft 40/승인 219 · 문제 draft 12/승인 3,199)와 **오류신고 월 35건**.
참고선: 학생 30일 신규 85 · 접속 110 · 문제 풀이 33 · staff 36 중 90일 무로그인 30.

| ID | 기능 | 우선순위 | 상태 |
|----|------|:-------:|:---:|
| feat-14-N1 | ★ **콘텐츠 제작·검수 처리량** — 검수 큐 화면(도식·문제·해설을 한 화면에서 승인/반려, 목록↔편집 왕복 제거) + 감사 스크립트 결과(`audit-diagrams` WARN·`audit-essay-answers` FAIL)를 큐 정렬 키로 + 정정 도구(find-replace) 연결. **네 축 전부의 원재료라 사슬이 붙어도 버려지지 않는다.** 지표=도식 draft 40→10 이하. | P0 | 🔲 |
| feat-14-N2 | **오류신고 루프** — 담당자·처리 메모·관련 화면 기록(현재 `resolution_note` 하나뿐, 답변자 귀속 필드 없음) + 같은 화면 URL 신고 묶기 + 처리 기간 측정 시작. 월 35건으로 Q&A(25)보다 유입이 많다. | P0 | 🔲 |
| feat-14-N3 | **학생 여정 상태 SSOT** — 가입→활성→학습 지속→반→응시→합격 을 파생 함수 `getStudentJourney` 하나로(신규 테이블 없음). 지금은 등급·반·수강권·구독·활동이 다섯 군데에 흩어져 화면마다 재계산. **내년에 '수강' 한 칸만 끼우면 네 축 통합이 새 설계가 아니라 확장이 된다.** | P1 | 🔲 |
| feat-14-N4 | **내년 오픈 준비 — 리드타임 확정(개발 없음)** — ①콜러스 fingerprint→ENFORCE_DEVICE ON[벤더] ②watch_events 보존 근거→처리방침[법무] ③T-PASS 연결 절차[원장, 강의 콘텐츠 제작 전 필요] ④토스·카카오 라이브 키[원장]+dunning 라이브 1회 검증 ⑤staff 계정 위생 30개[원장]. 오픈 시점 역산해 수령 기한만 정한다. | P0 | 🔲 |
| feat-14-보류 | **온보딩·활성화 캠페인 / 종합반 이행판 확대 / 관제(MRR·PG 대사·정산 지급·4-eyes)** — 온보딩은 유입 동기가 강의로 바뀌면 전제가 달라져 다시 만들게 된다. 관제는 실매출 후. 기능 부족이 아니라 **시점** 문제로 보류. | P2 | ⏸ |
