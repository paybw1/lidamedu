...
| feat-2-037 | **도해특허법 빈칸 학습 모드 (유형 1·2·3)** — 도해 유닛(94개)의 표·본문 낱말에 빈칸을 뚫되, **그 단원의 기출·정오문제에서 실제로 논의된 말만** 고른다. 원장 요청(2026-09-04). **용도는 1차 객관식** — 2차용 학습모드(판례 도식 feat-2-035 S6 · 모범답안 feat-2-036)와 목적이 달라 채점도 서술 커버리지가 아니라 **낱말 정오 판정**(조문 빈칸과 동일). 유형 1=기출 유래 · 유형 2=정오(OX) 유래 · 유형 3=합집합(★원장 지시문에 유형 정의가 없어 "원천 둘 + 1+2=합집합"으로 읽음 — 확인 필요). **조문 원문 박스는 제외**(조문 빈칸이 이미 있다) — ★블록 JSON 에는 남아 있고 렌더에서 갈아끼우는 것도 유닛당 첫 박스 하나뿐이라 **판정은 블록에서 직접** 한다(안 그러면 「조약의 효력」에서 교재 조문이 빈칸이 된다). 소제목(`h`)·도해 이미지도 제외. **구현 전 전수 실측**(`scripts/dohae/probe-blank-terms.ts`, 저장 없음): 말은 **문제 쪽에서 모으고 유닛 본문에서 찾는다**(도해 표 칸은 조사가 없어 `nounStem` 이 유닛 본문에선 아무것도 못 잡는다). 거르기 3종 — ①조사가 실제로 벗겨진 체언만(안 그러면 「공지된」·「받을」이 상위권, 「진보성/진보성이/진보성을」로 갈라짐) ②**코퍼스에 홀로 쓰인 적 있는 말만**(★「선출원주의」→`의` 를 떼면 「선출원주」. 「적용받」·「소급되」도 같이 걸림) ③문서빈도 8%(2글자는 2%) 상한(「발명」·「경우」는 답이 뻔하다). 결과 = 88/94 유닛에서 생성, 평균 30개(0개 6·1~4개 5), 실제 예: 최후거절이유통지·보정각하결정·신규사항·기재불비(t25) / 정정심판·확인대상발명·특허취소신청·권리범위확인심판(r6-2). ★**유형1·2 어휘는 많이 겹친다**(기출만 127·정오만 1,016·둘다 1,749 — OX 지문이 기출 선지에서 나오기 때문). 그래도 유닛당 상한을 두고 **유형1은 기출 등장 수·유형2는 OX 등장 수로 순위**를 매겨 뽑히는 12개가 갈린다. 저장 = `dohae_blank_terms`(unit_id·term·from_exam/from_ox·근거 건수·`excluded_at`) — ★**좌표는 저장하지 않는다**(조문 빈칸은 재직렬화 한 번에 96세트 좌표가 유실됐다; 도해 본문은 운영자 편집·재파싱 둘 다로 바뀐다). 위치는 렌더 시 본문에서 찾는다. 입력은 `case-blank-fill-view` 의 `CaseBlankInput` 패턴 그대로(V2 contenteditable 은 조문 전용이라 미이식) + **iPad 실기 확인이 완료 조건**. 빈칸 모드에서 하이라이트·포스트잇은 **렌더만 멈춘다**(저장 오프셋 불변 — 입력 칸이 글 흐름에 들어가면 어긋난 자리에 그어진다). 유출방지 4겹 유지. 기록·SRS 합류는 별건(`user_blank_srs` 는 `set_id` 매인 구조, `user_problem_attempts` 는 학습현황 신호 오염). 공개는 **staff 선출시 → 검수 후 학생**. 설계 `docs/features/feat-2-037-dohae-blank-practice.md`. | P2 | 🔲 |
