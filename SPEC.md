...
## 5.12 운영 통합 — 강의 · 학습 · 평가 · 컨설팅 — feat-14

네 축을 하나의 사슬로 잇는 운영 고도화. **새 기능이 아니라 가동·연결 작업이다** — 기능은 대부분 완성돼 있고
(운영관리 26 클러스터 111 화면 · LMS M1~M4 · 컨설팅 feat-7-047/048), 병목은 가동(adoption)과 축 사이 연결이다.
설계 SSOT: `docs/features/feat-14-ops-integration.md` (2026-09-01 제안, **승인 대기**).

진단 근거(운영 DB, 학생 한정 30일): 신규 85 · 접속 110 · **문제 풀이 33(전환 30%)** ·
오프라인 시험 **44건 전부 draft·응시 0** · 월간 계획 4/11 · 활성 수강권 2 · 결제 완료 주문 1 ·
90일 무로그인 staff 계정 30/36.

| ID | 기능 | 우선순위 | 상태 |
|----|------|:-------:|:---:|
| feat-14-G1 | **강의 오픈 게이트** — ENFORCE_DEVICE ON([벤더]) · watch_events 처리방침(법무) · T-PASS 연결 절차 · 토스/카카오 라이브 키 · dunning 라이브 청구 검증. 개발 소, **외부 결정 대기**. | P0 | 🔲 |
| feat-14-G2 | **학습 첫 행동 온보딩** — 첫 7일 "오늘의 한 가지" 트랙 + 3일 무행동 복귀 훅(세그먼트 메시징 재사용). 신규 테이블 없음. 지표=접속→첫 풀이 30%→50%. | P0 | 🔲 |
| feat-14-G3 | **오프라인 시험 배포 루틴** — 반 상세 "이번 주 시험" 카드(draft→배포 원클릭)+워크큐 타일. 기능은 완비, 루틴만 없음. | P0 | 🔲 |
| feat-14-G4 | **컨설팅 루프 이행판** — 반 상세에 학생별 (진단/계획/기록/상담) 4칸 신호등, 빈칸에서 바로 대신 제출·상담. 기존 조회 재배열. | P1 | 🔲 |
| feat-14-G5 | **staff 계정 위생** — 90일 무로그인 30계정 회수(admin 16 포함). role enum 불변, duty 유지. | P1 | 🔲 |
| feat-14-C1 | ★ **학생 여정 상태 SSOT** — 가입→체험→수강→반→응시→합격 을 파생 함수 `getStudentJourney` 하나로(신규 테이블 없음). CRM 헤더·체험전환·세그먼트·이탈경보가 같은 정의를 쓰게 통일. **네 축을 하나의 제품으로 묶는 핵심.** | P0 | 🔲 |
| feat-14-C2 | **시청→파악 합류** — watch_events/lesson_completions 를 학습현황 활동 신호에 **별도 축으로 병기**(문제 진도와 합치지 않음). G1 선행. | P1 | 🔲 |
| feat-14-C3 | **평가→컨설팅 표준화** — 상담 화면에 최근 평가 3건(모의·오프라인·GS) 요약 + 약점→과제 CTA 직결. | P1 | 🔲 |
| feat-14-C4 | **회원 등급 단일화** — getMembershipAccess ↔ enrollments/user_subscriptions 를 C1 여정 상태에 흡수. | P1 | 🔲 |
| feat-14-P3 | **관제(별도 승인)** — MRR·이연매출·마진 / PG 대사 큐 / 정산 지급 실행·명세·분쟁 / 4-eyes 승인 / 크로스반 헬스. **실매출 발생 후 착수**(0 위의 대시보드 방지). | P2 | 🔲 |
