...
| feat-2-036 | **주관식 모범답안 연습 모드 (목차 연습 · 내용 연습)** — 판례 도식 S6(feat-2-035 §10)의 "가리고 복원"을 2차 주관식 모범답안에 적용. 원장 요청(2026-09-04)대로 **① 목차 연습**(발문만 보고 목차를 세운다)과 **② 내용 연습**(목차는 보이고 잎 칸 본문이 빈칸)으로 나눈다 — 목차와 내용은 틀리는 방식이 다르므로 섞으면 무엇을 고칠지 알 수 없다. ★기존 3단계 훈련(feat-2-032 S5)과 중복 아님: 그쪽은 **백지에서** 스스로 세우고 AI 채점, 이쪽은 **모범답안을 복원**하고 즉답·무료 채점 — 백지 훈련 전 단계의 사다리. 전수 실측 184건: `##` 없는 답안 0건, 중앙 설문 3·중목차 11·소목차 9·3,568자 → **설문(`##`) 단위로 끊는다**(전체는 20칸이라 아무도 안 끝낸다). 채점은 `answer-match.ts` 공용(새로 만들지 않음), 임계값 0.65/0.35 그대로 — **짧은 제목에도 통하는지 구현 전 실측**: 뼈대 같은 목차 0.70 / 큰 뼈대만 0.19 / 다른 논점 0.04 (도식과 같은 자리, `outline-practice-probe.test.ts`). 순서는 별개 신호로만 표시하고 감점 없음(실측 없는 감점 금지). 저장은 v1 localStorage — `user_subjective_attempts` 에 넣으면 3단계 훈련 KPI 가 부풀어 완주율이 뜻을 잃는다. ★**결정 대기**: 주관식 뷰어가 지금 staff 전용(`problem-viewer.tsx:233` + `redactSubjectiveAnswer`)이라, 학생 공개는 연습 모드와 별개의 콘텐츠 공개 결정 — 권고는 staff 전용 선출시. 설계 `docs/features/feat-2-036-essay-answer-practice.md`. | P2 | 🔲 |
