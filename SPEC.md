...
| feat-11-012 | **강의 플랫폼 오픈 전 정비(260913 체크리스트).** 설계=`docs/features/feat-11-012-lecture-open-readiness-260913.md`. 조사 8축 160건 → 중복 정리 60여 건 → 묶음 12개. **성격**: 기능 추가가 아니라 「이미 만들어 둔 것이 화면에 닿게」 — 공용 부품을 만들거나 되찾아 N건을 한 번에 닫는다. P0 게이트 전제(★학생 게이트 4종이 /lecture 에 안 걸림 — private.layout 미중첩) / P1 검색 노출(사이트맵 7건 전부 이중 슬래시·강의 경로 0건, canonical 0곳, lang=en, next 무시) / P2 폰(모바일 탭 20개 평탄화 한 줄·헤더 최소폭 480→374·구매바 셸·스와이프 훅) / P3 첫 화면(배너 0건이면 h1 0개 → 소개 히어로 폴백·하드코딩 숫자 실측화) / P4 결정 정보(강사·회차 공급자 확장·본문 채우면 포함강의 사라지는 분기 버그·PriceTag) / P5 결제(세트 한 줄·바로구매 복귀 분기·결제결과 단일 리더·금액 권위 서버·쿠폰 KST·포인트 잔액 DB) / P6 산 다음(재생 판정 단일화·하트비트 소진 가드·EmptyState 되찾기·표기 SSOT) / P7 잔손질(확인 다이얼로그 2파일·공용 페이저). ★원장 결정 대기: D1 종이책 / D2 무통장 / D3 게이트 범위 / D4 도메인 단일화 / D9 학생 게이트 적용 구간 / D10 ENFORCE_DEVICE(충돌한 두 결정) / D11 후기 노출 / D12 진도율 분자. | P0 | 🔲 |
