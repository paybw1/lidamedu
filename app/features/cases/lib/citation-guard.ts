...
const CASE_MARKS = new Set([
  ...
  // 헌법재판소 결정 — 2026-09-12 추가. `markOf` 는 앞뒤 숫자만 떼므로 "2020헌바497" → "헌바".
  // ★이건 Set 이라 **나열 순서가 판정에 영향을 주지 않는다**(알터네이션이 아니다).
  //   맨 "헌" 단독 부호는 존재하지 않아 앞뒤로 가려지는 문제도 없다.
  // ★★실재 판정이 casenote **단독**에 의존한다(2026-09-12 실측: 2020헌바497·89헌마160 은
  //   casenote=true · 법령정보센터 target=prec=false — prec 는 법원 판례만 담는다).
  //   casenote 가 막히면 헌재 번호는 전부 "지어냄" 으로 잡히니, FAIL 이 뜨면 소스별로 볼 것.
  "헌마", "헌바",
]);
