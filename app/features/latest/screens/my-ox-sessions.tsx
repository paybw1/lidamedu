...
function SessionRow({
  ...
  const rate = answered > 0 ? Math.round((s.correct / answered) * 100) : 0;
  return (
    <Link
      to={s.packId ? `/latest/mcq/${s.packId}/ox-exam` : "#"}
      className="bg-card hover:border-primary/40 hover:bg-muted/40 block rounded-xl border p-3.5 shadow-sm transition-colors"
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          ...
          <div className="text-muted-foreground mt-1 flex flex-wrap items-center gap-1.5 text-[11px]">
            ...
            <span className="tabular-nums">{fmtDate(s.completedAt ?? s.startedAt)}</span>
