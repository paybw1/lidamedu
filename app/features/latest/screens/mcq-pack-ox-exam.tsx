...
export default function McqPackOxExam({ loaderData }: Route.ComponentProps) {
  ...
  const desc =
    mode === "study"
      ? `문제집의 ${pack.problemCount}개 문항에서 정오문제 지문 ${items.length}개를 추출했습니다. 답을 누르면 즉시 정답·해설이 표시됩니다. 응시 이력에는 남지 않지만, 지문별 정오는 학습 분석에 반영됩니다.`
      : `문제집의 ${pack.problemCount}개 문항에서 정오문제 지문 ${items.length}개를 추출했습니다. 모두 풀고 제출하면 채점 결과를 표시하고 응시 이력에 저장합니다.`;
  return (
    <McqAreaShell
      isMock
      width="feed"
  ...
  return (
    <div className="border-border bg-card rounded-2xl border p-8 text-center shadow-sm">
      <p className="text-foreground font-semibold">
        이 문제집에 시험 가능한 정오문제 지문이 없습니다
      </p>
      <p className="text-muted-foreground mt-2 text-sm">
        문제집 문제의 보기·박스 항목에 정답이 설정되어 있지 않거나, 모두 정오문제
        평가 부적합으로 표시되어 있습니다.
      </p>
      <Button asChild size="sm" className="mt-4">
        <Link to={`/latest/mcq/pack/${packId}`}>
          <ArrowLeftIcon className="size-3.5" /> 문제집 상세로
        </Link>
      </Button>
    </div>
  );
