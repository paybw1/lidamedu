// 최신 객관식 문제 — 등록일 최신 50건. 카드별 "지금 풀어보기" CTA.

import { ArrowRightIcon, ListChecksIcon, NewspaperIcon } from "lucide-react";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const problems = await listRecentProblems(client, 50, "mcq");
  return { problems };
}
  ...
  return (
    <div className="mx-auto w-full max-w-screen-lg px-5 py-6 md:px-10 md:py-8">
      <header className="mb-6 space-y-2">
        ...
        <h1 className="text-2xl font-bold tracking-tight">객관식 문제</h1>
        <p className="text-muted-foreground text-sm">
          등록일 기준 최신 {problems.length}건 (mc/ox/blank 통합)
        </p>
      </header>
      ...
      {problems.length === 0 ? (
        ...
        <div className="space-y-2" data-testid="latest-mcq-list">
          {problems.map((p) => (
            <Link
              key={p.problemId}
                ...
                <CardHeader className="px-4 pb-2">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <Badge variant="default" className="text-xs">
                      <ListChecksIcon className="size-3" /> {lawName(p.lawCode)}
                    </Badge>
                    <Badge variant="secondary" className="text-xs">
                      {ORIGIN_LABEL[p.origin as ProblemOrigin] ?? p.origin}
                    </Badge>
                    <Badge variant="outline" className="text-xs">
                      {FORMAT_LABEL[p.format as ProblemFormat] ?? p.format}
                    </Badge>
                    {p.year ? (
