...
import { ArrowRightIcon, NotebookPenIcon, PlayIcon, TimerIcon } from "lucide-react";
...
import { listWrongAttempts } from "~/features/study/queries.server";
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const items = await listWrongAttempts(client, user.id, subject);
  return { items, subject: subject ?? null };
}
    ...
    <div className="mx-auto w-full max-w-screen-lg px-5 py-6 md:px-10 md:py-8">
      <header className="mb-6 space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          ...
          가장 최근 시도가 오답인 문제만 노출됩니다. 다시 풀어 정답이면 자동 제거됩니다.
        </p>
      </header>
      ...
      {items.length > 0 ? (
        ...
        >
          <div className="flex items-center gap-2 text-sm">
            ...
            오답 {items.length}건을 한 세션으로 묶어 풀기
          </div>
          {subject ? (
            ...
            >
              <Card className="hover:border-primary transition-colors">
                ...
                <CardContent className="px-4 pb-4">
                  <p className="text-sm leading-relaxed">{item.bodySnippet}</p>
