...
export async function action({ request }: Route.LoaderArgs) {
  ...
  if (message) {
    ...
    // console.log("📧 Parsed email message → To:", to, "| Template:", template);
    // console.log("📦 Email data:", emailData);

    // message가 배열이므로, 첫 번째 요소만 꺼내야 함
    const raw = message[0];
    if (!raw || !raw.message) {
      return data(null, { status: 200 });
    }

    const { to, data: emailData, template } = raw.message;
    ...
    if (template === "welcome") {
      ...
      const { error } = await resendClient.emails.send({
        ...
        from: "Brandit Support <support@brandit-ip.com>",
        to: [to],
        subject: "Welcome to Brandit!",
