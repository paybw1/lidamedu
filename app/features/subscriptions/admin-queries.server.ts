...
export async function listAllSubscriptions(
  ...
  let q = adminClient
    .from("user_subscriptions")
    .select(
      "subscription_id, user_id, plan_id, started_at, expires_at, status, payment_id, subscription_plans!inner(code, name), profiles!inner(profile_id, name)",
    )
    .order("started_at", { ascending: false })
...
> {
  ...
  if (!plan.is_active) return { ok: false, error: "비활성 플랜" };
...
export async function extendSubscription(
  ...
  const update: { expires_at: string; status?: SubscriptionStatus } = {
