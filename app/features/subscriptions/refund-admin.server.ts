...
//
// ★feat-11-013 D10(2026-09-15) 이후 이 파일이 **학습 플랫폼 구독 환불의 유일한 경로**다.
// 종전에는 학생 셀프 해지(`cancelSubscription`)가 결제 후 3일 이내를 스스로 환불했으나,
// 지금은 그 갈래가 거부되고 고객센터 신청 → 운영자 처리로 바뀌었다(환불정책 제3조 1항·제5조 1항).
// 운영자는 기간 제한 없이 환불한다.
//
...
export async function refundPaymentAdmin(input: {
  ...
  // ★★feat-11-013 D9 — 리담 관리자페이지가 토스 취소 API 를 **자동 호출하지 않는다**(요청서 §5).
  //   단, 막는 범위는 **주문이 있는 결제**(강의 플랫폼)뿐이다. 그쪽은 /admin/refunds 가
  //   접수 → 검토 → 금액확정 → 관리자가 토스에서 직접 취소 → 취소정보 입력 순서로 받는다.
  //
  //   ★주문 없는 결제(학습 플랫폼 구독)는 **여기서 막으면 안 된다.** 실측 2026-09-14:
  //   완료 결제 22건 중 18건이 주문 없는 구독 결제이고, 역대 환불 6건 중 4건이 그쪽이다.
  //   환불관리 모델은 refunds.order_id 가 필수라 그 건들을 담을 수 없다 — 막는 순간
  //   학습 플랫폼 구독의 환불 수단이 통째로 사라진다. 요청서도 강의 플랫폼 문서다.
  if (pay.order_id && pay.toss_payment_key) {
    return {
      ok: false,
      error:
        "주문 결제는 환불관리에서 처리합니다. 주문관리에서 [환불신청]으로 접수한 뒤, 토스 상점관리자에서 직접 취소하고 취소정보를 입력해 주세요.",
    };
  }
