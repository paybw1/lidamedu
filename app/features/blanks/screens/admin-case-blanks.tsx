// feat-2-029 S5 — 판례 빈칸 후보 승인 큐. OX 기출 유래 AI 후보(case_blank_candidates)를
// 검토·승인하면 판례의 '기출 유래' 세트(case_blank_sets)에 blank 로 기록된다.
// admin-ox-review 의 REVIEW QUEUE 패턴 — AdminShell + IndexTable + 상태 chip + 행 단위 fetcher.
...
import {
  countCaseBlankCandidates,
  listCaseBlankCandidates,
  type CaseBlankCandidateRow,
  type CaseCandidateStatus,
} from "~/features/blanks/case-candidates.server";
import { getStaffRole } from "~/features/laws/queries.server";

import type { Route } from "./+types/admin-case-blanks";

export const meta: Route.MetaFunction = () => [
  { title: "판례 빈칸 승인 | 리담변리사학원" },
];

const STATUS_OPTIONS: {
  value: CaseCandidateStatus;
  label: string;
  hint: string;
}[] = [
  {
    value: "pending",
    label: "대기",
    hint: "AI 후보 — 승인 전 학생 미노출",
  },
  { value: "approved", label: "승인됨", hint: "세트에 기록된 빈칸" },
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const status: CaseCandidateStatus =
    statusParam === "approved" || statusParam === "rejected"
      ? statusParam
      : "pending";

  const [items, counts] = await Promise.all([
    listCaseBlankCandidates(client, status),
    countCaseBlankCandidates(client),
  ]);
  return { status, items, counts, role };
}

export default function AdminCaseBlanks({ loaderData }: Route.ComponentProps) {
  const { status, items, counts, role } = loaderData;
  const [searchParams] = useSearchParams();

  // 판례별 그룹 헤더 행을 끼워 넣기 위한 sentinel(admin-blanks-list 패턴).
  type RowSentinel =
    | { kind: "group"; item: CaseBlankCandidateRow; count: number }
    | { kind: "row"; item: CaseBlankCandidateRow };
  const renderRows: RowSentinel[] = [];
  let lastCaseId: string | null = null;
  for (const it of items) {
    if (it.caseId !== lastCaseId) {
      renderRows.push({
        kind: "group",
        item: it,
        count: items.filter((x) => x.caseId === it.caseId).length,
      });
      lastCaseId = it.caseId;
    }
  ...
  return (
    <AdminShell
      ...
      desc={
        <>
          판례에 연결된 기출의 <span className="font-semibold">거짓(X) 지문</span>
          에서 AI가 도출한 함정 핵심어 후보입니다. 승인하면 그 판례의{" "}
          <span className="font-semibold">'기출 유래' 빈칸 세트</span>에
          기록되고 판례 뷰어 ①빈칸 모드에 나타납니다. 승인 전에는 학생에게
          노출되지 않습니다.
        </>
      }
    >
      ...
      {items.length === 0 ? (
        <div className="border-border bg-card rounded-xl border py-16 text-center shadow-sm">
          ...
          <p className="text-muted-foreground text-sm">
            {status === "pending"
              ? "대기 중인 후보가 없습니다. 후보 생성은 scripts/cases/gen-case-blank-candidates.mjs 로 합니다."
              : "해당 상태의 후보가 없습니다."}
          </p>
        </div>
      ) : (
        ...
        >
          {renderRows.map((r) =>
            r.kind === "group" ? (
              <tr key={`g-${r.item.caseId}`} className="bg-muted/40">
                <td colSpan={4} className="px-3 py-1.5">
                  ...
                  {r.item.subjectSlug ? (
                    <Link
                      to={`/subjects/${r.item.subjectSlug}/cases/${r.item.caseId}`}
                      viewTransition
...
/* ── 후보 행 — 정답 인라인 수정 + 승인/거절/되돌리기 ─────────────────────── */

function targetLabel(item: CaseBlankCandidateRow): string {
  if (item.target === "reasoning") return "판시이유";
  if (item.target === "comment") return "평석";
  return `요지 #${(item.itemIndex ?? 0) + 1}`;
}

function CandidateRow({ item }: { item: CaseBlankCandidateRow }) {
  ...
  const submit = (op: "approve" | "reject" | "revert") => {
    ...
    fetcher.submit(fd, {
      method: "post",
      action: "/api/blanks/case-candidate-review",
    });
  };
  ...
  return (
    <TR>
      <TD soft>
        <Chip tone="neutral">{targetLabel(item)}</Chip>
      </TD>
      <TD>
        ...
        {answer.trim() !== item.answer ? (
          <p className="text-muted-foreground mt-0.5 text-[10px]">
            원안 "{item.answer}" 에서 수정됨 — 원문에 그대로 있는 표현이어야
            승인됩니다.
          </p>
        ) : null}
        {error ? (
