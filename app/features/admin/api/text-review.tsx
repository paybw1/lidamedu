// feat 자과 지문 텍스트 전사 검수 API. intents:
//   approve      → RPC approve_text_draft (도표=재크롭이미지 / 순수텍스트=발문텍스트 를 body_md 로,
//                  선지 텍스트를 problem_choices.body_md 로 복사 + status=approved)
//   reject       → 초안 status=rejected (+ note)
//   bulk-approve → 다중 draftId 일괄 승인
// 게이트: staff(instructor/manager/admin) 만. RLS 보호된 supa-client(RPC 가 auth.uid() 로 staff 확인).
...
import type { Route } from "./+types/text-review";
...
export async function action({ request }: Route.ActionArgs) {
  ...
  if (intent === "approve") {
    ...
    const { error } = await client.rpc("approve_text_draft", {
      p_draft_id: id.data,
    });
