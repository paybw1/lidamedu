-- feat-11-013 P7-핸드오프 ③ 보강 — 결제귀속액 폴백을 정확하게
--
-- 20260915_p7h3 은 paid_amount_krw 가 없으면 **정가**로 폴백했다. 정가는 쿠폰·포인트를
-- 빼기 전 금액이라, 스냅샷이 없는 옛 주문에 쿠폰이 붙어 있으면 분모가 과대평가돼
-- 환산값이 작아진다(정산이 실제보다 적게 환불된 것으로 읽는다 = 강사에게 과소 공제).
--
-- 실측 2026-09-15: paid_amount_krw 는 61건 전부 비어 있고(P1 이후 신규 주문이 없다 —
-- 강의 게이트가 닫혀 있다), 쿠폰 주문·포인트 주문은 0건이라 **지금 틀어진 행은 없다.**
-- 그래도 폴백을 paid_amount_krw 의 정의(정가 − 쿠폰배분 − 포인트배분)와 같게 맞춘다.
-- coupon_alloc_krw·point_alloc_krw 는 NOT NULL default 0 이고 61/61 채워져 있다.
--
-- 적용: node scripts/run-prod-sql.mjs scripts/sql/20260915_p7h3b_paid_fallback.sql
