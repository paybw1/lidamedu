...
-- 결과 4. 노출본 vs 삭제분 + 빈도 등급 분포
SELECT
  count(*)                                              AS not_in_db_total,
  count(*) FILTER (WHERE live_hits > 0)                 AS appears_in_live,
  count(*) FILTER (WHERE live_hits = 0)                 AS deleted_only,
  count(*) FILTER (WHERE source_blind)                  AS blind_total,
  count(*) FILTER (WHERE freq_class = 'singleton')      AS c_singleton,
  count(*) FILTER (WHERE freq_class = 'single_row')     AS c_single_row,
  count(*) FILTER (WHERE freq_class = 'low')            AS c_low,
  count(*) FILTER (WHERE freq_class = 'recurring')      AS c_recurring
FROM scored WHERE NOT in_db;
