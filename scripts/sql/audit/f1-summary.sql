...
-- 결과 1. 부호별 요약
SELECT code,
  bool_or(mark_in_tool)                                          AS in_tool_marks,
  count(*)                                                       AS distinct_cited,
  count(*) FILTER (WHERE in_db)                                  AS in_db,
  count(*) FILTER (WHERE NOT in_db)                              AS not_in_db,
  count(*) FILTER (WHERE NOT in_db AND source_blind)             AS not_in_db_blind,
  count(*) FILTER (WHERE NOT in_db AND NOT source_blind
                        AND freq_class = 'singleton')            AS verify_targets
FROM scored GROUP BY code
HAVING count(*) FILTER (WHERE NOT in_db) > 0
ORDER BY not_in_db DESC;
