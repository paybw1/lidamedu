...
-- 결과 2. --verify 투입 목록(출처맹점 제외분). 허·후는 f3 에서 따로 받는다.
SELECT case_no, code, total_hits, distinct_rows, live_hits, srcs, source_blind
  FROM scored
 WHERE NOT in_db AND freq_class IN ('singleton','single_row')
 ORDER BY source_blind, live_hits DESC, case_no;
