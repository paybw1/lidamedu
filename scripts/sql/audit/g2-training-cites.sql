...
-- 2차 훈련 논점이 실제로 내놓는 사건번호 — 종수·DB수록 여부·다른 소스와의 공유 여부
, t AS (
  SELECT c.case_no, c.code, count(*) AS hits_in_training
    FROM court c WHERE c.src = '2차 훈련 논점' GROUP BY 1,2
)
SELECT t.case_no, t.code, t.hits_in_training,
       s.in_db, s.source_blind, s.freq_class, s.distinct_rows, array_to_string(s.srcs,'+') AS srcs
  FROM t JOIN scored s USING (case_no)
 ORDER BY s.in_db, t.case_no;
