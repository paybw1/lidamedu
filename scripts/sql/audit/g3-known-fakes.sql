...
-- 2026-09-01 확인된 환각 3건이 지금도 남아 있는가.
--   ★scored 는 화이트리스트 통과분만 담으므로, 원시 추출(cited)과 본문 LIKE 양쪽으로 본다.
, want(no) AS (VALUES ('2005후3352'),('2009후3919'),('2015다257538'))
, in_cited AS (
  -- ★count(*) 이 아니라 count(c.case_no) — LEFT JOIN 미매칭 행을 1 로 세면 안 된다.
  SELECT w.no, count(c.case_no) AS hits, count(DISTINCT c.row_id) AS rows_,
         array_agg(DISTINCT c.src) AS srcs
    FROM want w LEFT JOIN cited c ON c.case_no = w.no
   GROUP BY w.no
)
, in_raw AS (  -- 정규식과 무관하게 본문 문자열로도 훑는다(공백 낀 표기 대비)
  SELECT w.no,
         (SELECT count(*) FROM corpus b
           WHERE regexp_replace(coalesce(b.body,''), '\s', '', 'g') LIKE '%' || w.no || '%') AS body_rows
    FROM want w
)
SELECT c.no, c.hits, c.rows_, c.srcs, r.body_rows,
       (SELECT count(*) FROM known k WHERE k.case_no = c.no) AS in_db_known
  FROM in_cited c JOIN in_raw r USING (no) ORDER BY c.no;
