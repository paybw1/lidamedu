...
-- 결과 2-요약. 투입 후보를 소스·부호별로
SELECT array_to_string(srcs, '+') AS src_set, code, count(*) AS n
  FROM scored
 WHERE NOT in_db AND NOT source_blind AND freq_class IN ('singleton','single_row')
