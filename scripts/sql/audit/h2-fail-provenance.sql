...
SELECT
  CASE WHEN 'cases-only' = CASE WHEN s.srcs = ARRAY['판례 서술']::text[] THEN 'cases-only' ELSE 'x' END
       THEN '판례 서술 단독(판결문 원문)'
       WHEN '판례 서술' = ANY(s.srcs) THEN '혼합(판례 서술 포함)'
       ELSE 'AI 저작면 단독' END                         AS provenance,
