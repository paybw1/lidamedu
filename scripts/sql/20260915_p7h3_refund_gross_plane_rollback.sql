-- feat-11-013 P7-핸드오프 ③ — 부분 금액 환불의 정가 평면 환산 (롤백 — 정가 전액으로 되돌림)
--
-- ★돈 버그였다. saveRefundAmounts 는 항목별 final_krw 를 0원~결제액 사이 아무 값이나
--   받는데(공제사유 칸까지 있다), commit_refund 는 order_items.refund_amount_krw 에
--   **항상 정가 전액**(unit_price × quantity)을 써 왔다. 정산은 그 값을
--   scaleRefund(값, 할인후, 정가) 로 읽으므로 **전액 환불로 계산**된다 —
--   학원은 차액을 쥐고 있는데 강사 정산에서만 그 항목이 통째로 빠진다.
--
--   적용 시점(2026-09-15) refunds·refund_items 0건 — 정정할 과거 데이터 없음.
--
-- 적용: node scripts/run-prod-sql.mjs scripts/sql/20260915_p7h3_refund_gross_plane.sql
