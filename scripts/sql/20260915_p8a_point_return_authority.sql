-- feat-11-013 P8-a — 포인트 반환 권위 통일.
--
-- ★요청서 11-11 은 「포인트 반환액 = MIN(사용 포인트, 환불 대상금액)」을 못 박는다.
--   엔진(refund-calc.ts)은 그대로 계산하는데, **실제로 포인트를 움직이는** 이 RPC 는
--   `point_alloc_krw` **전액**을 무조건 돌려주고 있었다. 전량 환불에서는 두 값이 같아
--   드러나지 않지만, 부분 환불에서 갈린다 —
--     포인트 30,000 + 카드 70,000 → 환불 대상금액 20,000 일 때
--     화면은 「포인트 20,000」이라 말하고 학생은 **30,000** 을 받는다.
--
-- 딸린 문제 둘을 같이 막는다.
--   ① 잔액 영구 동결 — 멱등 축이 (kind, ref_type='order_item', ref_id) 라 **항목당
--      restore 1행**뿐이었다. 부분 반환하면 나머지는 두 번째 환불에서 영영 못 돌려준다.
--      `refund_items_open_uidx` 가 **열린 건에만** 유일하므로 순차 재환불은 실제로 가능하다.
--   ② 잔여 합산 불가 — point_transactions 에 항목 식별자가 없어 「이 항목으로 이미
--      얼마를 돌려줬나」를 셀 수 없었다.
--
-- 적용 시점 근거: 운영 노출 0(포인트 결제 항목 0건 · restore 0행 · 환불 1건 미확정).
-- D15(포인트 결제)를 열고 나면 학생 돈이 걸린 이관이 된다.
