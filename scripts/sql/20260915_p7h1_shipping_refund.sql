-- feat-11-013 P7-핸드오프 ① — 배송비 환불 자리 (적용)
--
...
-- ① 배송비를 돌려줄 자리 — 주문 헤더의 배송비는 order_items 에 없으므로 refunds 헤더에 둔다.
alter table public.refunds
  add column if not exists shipping_refund_krw integer not null default 0;

comment on column public.refunds.shipping_refund_krw is
  '이번 환불로 돌려주는 배송비(원). orders.shipping_fee_krw 에서 이미 돌려준 몫을 뺀 만큼이 상한. '
  '★반품비 공제와 다른 돈이다 — 반품비는 refund_items.shipping_deduction_krw(P7 엔진 소관).';

alter table public.refunds drop constraint if exists refunds_shipping_refund_nonneg_check;
alter table public.refunds add constraint refunds_shipping_refund_nonneg_check
  check (shipping_refund_krw >= 0);
