...
const GENERAL_WORDS = new Set([
  ...
  "방지", "보통", "암시",
  // 남은 두 글자 말 158종을 문맥과 함께 검수한 뒤 원장이 고른 것(2026-09-11). 세 관점 판정에서
  // 빼기 제안 15 + 경계 3(사업·납부·체결) + 유지였던 3(피고·합병·화해). 「기재방법」은 t21 한 자리의
  // 문제(행 라벨에 답이 보임)라 상수가 아니라 그 유닛의 excluded_at 으로 뺐다.
  "적용", "지급", "예외", "질병", "구조", "분쟁", "공급", "비용", "성립", "실험",
  "인간", "종래", "치료", "연구", "조정", "사업", "납부", "체결", "피고", "합병",
  "화해",
]);
