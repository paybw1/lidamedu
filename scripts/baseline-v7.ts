// baseline-v7 — production 변경 0, 측정 전용.
//
// 실행 :
//   AI_QNA_DAILY_COST_USD_CAP=8 npx tsx scripts/baseline-v7.ts
//
// 흐름 :
...
//   5) 저장 : rag-lab/eval/reports/baseline-v7-<ts>.jsonl + docs/eval/baseline-v7.md
//
...
async function main(): Promise<void> {
  process.stdout.write("=== baseline-v7 measurement ===\n");
...
async function writeAndAnalyze(
  ...
  const jsonlPath = join(EVAL_DIR, "reports", `baseline-v7-${ts}.jsonl`);
  ...
  // ── md 리포트 ──
  const md: string[] = [];
  md.push(`# AI Q&A — baseline v7 (v7-A 청킹 fix 후) (production 변경 0, 측정 전용)`);
  md.push(``);
  md.push(`- 생성: ${ts}`);
  md.push(`- 상태: ${status}`);
  ...
  md.push(`- 신규 파일: scripts/baseline-v7.ts (측정 스크립트만)`);

  const mdPath = join(DOCS_EVAL, "baseline-v7.md");
