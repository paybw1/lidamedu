// 특허 기출/기출변형 해설 도해 교체 — patent-figure-mapping.json 기반.
...
const SRC_DIR = "source/특허법 기출(1차)/＊객관식 해설 그림파일/1. 기출";
const BUCKET = "problem-explanations";
const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);

const mapping = JSON.parse(readFileSync("tmp/jagwa/patent-figure-mapping.json", "utf8"))
...
for (const [problemId, items] of byProblem) {
  const { data: p, error } = await sb
    .from("problems")
    .select("problem_id, year, problem_number, explanation_md, problem_choices(choice_id, choice_index, explanation_md), problem_box_items(box_item_id, marker, explanation_md)")
    .eq("problem_id", problemId)
    .single();
  if (error) { console.log(`FAIL 조회 ${problemId}: ${error.message}`); failed++; continue; }

  for (const m of items) {
    // loc 에 해당하는 필드에서 oldUrl 존재 확인
    ...
    if (!holder || !holder.includes(m.oldUrl)) {
      console.log(`MISS ${p.year}#${p.problem_number} ${m.file} — loc=${m.loc} 에 oldUrl 없음`);
      failed++;
      continue;
    }
    checked++;
    ...
    if (e2) { console.log(`FAIL 치환 ${p.year}#${p.problem_number}: ${e2.message}`); failed++; continue; }
    backup.push({ year: p.year, no: p.problem_number, problemId, loc: m.loc, file: m.file, oldUrl: m.oldUrl, newUrl });
    // 다음 항목이 같은 필드를 다시 읽도록 로컬 상태 갱신
    ...
    applied++;
    console.log(`OK ${p.year}#${p.problem_number} ${m.loc} ← ${m.file}`);
  }
}
if (APPLY) writeFileSync("tmp/jagwa/patent-figure-replace-backup.json", JSON.stringify(backup, null, 1));
console.log(`검증통과 ${checked} · 적용 ${applied} · 실패/누락 ${failed}${APPLY ? " · 백업 tmp/jagwa/patent-figure-replace-backup.json" : " (dry-run)"}`);
