...
12. ★★**거짓 정보를 만들지 않는다 — 특히 식별자(사건번호·조문번호·인용).** 환각은 법리가 아니라 **번호**에 붙는다. 2026-09-01 실제 사고: 2차 훈련 논점의 인용 사건번호 6건 중 **3건이 실재하지 않았다**(법리 서술은 맞아 읽어서는 안 잡힘. 정확한 판례가 이미 DB 에 있는데도 없는 번호를 만들어 냈다).
    - 사건번호는 **기억에서 꺼내 쓰지 않는다.** 네 곳에서 확인한다 — ① `cases` ② `case_lower_courts`(하급심) ③ 리담 교재 ④ 국가법령정보센터(`lawSearch.do?...&search=1&query=<번호>`, **사건번호 정확일치만 실재로 인정**). 넷 다 없으면 **번호를 빼고 법리만 서술**한다.
    - **AI 초안은 전량 검증 대상** — 한 곳에서 발견되면 같은 검사를 나머지 콘텐츠에도 돌린다. 전수 도구 = `scripts/audit/audit-case-citations.mjs --verify --publish`(DB → 법령정보센터 3단계 판정, 결과는 `/admin/review` 검수 큐 배지).
    - 조회 실패(네트워크 등)를 "없음"으로 단정하지 않는다. **특허심판원 심판번호(당·원·정·취·소·재)는 법원 판결이 아니므로 조회 대상에서 제외**한다(빼지 않으면 전부 오탐).
...
- 네 곳(① `cases` DB ② `case_lower_courts` DB ③ 교재 ④ 국가법령정보센터) 어디에서도 확인되지 않는 사건번호 인용. 하나라도 확인되면 번호를 그대로 살리고, 모두 없으면 동일 취지의 DB 수록 판례로 대체하거나 사건번호 없이 법리만 서술한다.
  - ★**하급심 사건번호(`~나~`·`~가합~`·`~허~`)는 `cases` 에 없다** — 상고심에 딸린 `case_lower_courts.lower_case_number` 로 적재돼 있다. `cases` 만 조회하고 "확인 안 됨"으로 결론내면 이미 갖고 있는 판결문을 놓친다(2026-08-24 2023나11436 실사례).
